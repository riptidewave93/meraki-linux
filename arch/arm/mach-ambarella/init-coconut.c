...
static struct platform_device *ambarella_devices[] __initdata = {
	...
	&ambarella_ambxman_audio_device,
	&ambarella_ambcloudcam_audio_device,
	&ambarella_idc0,
	&ambarella_idc1,
	&ambarella_idc0_mux,
	&ambarella_ir0,
...
static struct ambarella_key_table coconut_keymap_cloudcam[AMBINPUT_TABLE_SIZE] = {
	{AMBINPUT_GPIO_KEY,	{.gpio_key	= {KEY_WLAN,	0,	0,	GPIO(90),	IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING}}},
	{AMBINPUT_GPIO_KEY,	{.gpio_key	= {KEY_POWER,	1,	0,	GPIO(91),	IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING}}},
	{AMBINPUT_END},
};

static struct ambarella_key_table coconut_keymap_xman[AMBINPUT_TABLE_SIZE] = {
	{AMBINPUT_GPIO_KEY,	{.gpio_key	= {KEY_WLAN,	0,	0,	GPIO(91),	IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING}}},
	{AMBINPUT_GPIO_KEY,	{.gpio_key	= {KEY_POWER,	0,	0,	GPIO(90),	IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING}}},
	{AMBINPUT_END},
};
...
static void __init ambarella_init_coconut(void)
{
	...
	if (AMBARELLA_BOARD_TYPE(system_rev) == AMBARELLA_BOARD_TYPE_EVK) {
		...
		switch (AMBARELLA_BOARD_REV(system_rev)) {
			case 10:
				...
				coconut_board_input_info.pkeymap = coconut_keymap_cloudcam;

				ambarella_platform_adc_temper_controller0.adc_temper_channel = ADC_CH1;
			...
			case 20:
				...
				coconut_board_input_info.pkeymap = coconut_keymap_xman;
	...
	/* Register devices */
#if defined(CONFIG_CODEC_AMBARELLA_WM8974)
        ambarella_init_wm8974(0, 0x1A);
#endif  /* Register devices */
#if defined(CONFIG_CODEC_AMBARELLA_WM8940)
	ambarella_init_wm8940(0, 0x1A);
#endif	/* Register devices */
#if defined(CONFIG_CODEC_AMBARELLA_ALC5633)
	ambarella_init_alc5633(0, 0x1C);
#endif
