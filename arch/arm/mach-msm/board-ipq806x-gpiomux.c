...
static struct of_device_id ipq806x_gpiomux_ids[] __initdata = {
	{ .compatible = "ipq806x,gpiomux", },
	{}
};

#define BOARD_IPQ80806X_NGPIO 68
static struct msm_gpiomux_dt_config msm_dt_cryptid_gpiomux[BOARD_IPQ80806X_NGPIO];
static void ipq806x_dt_gpiomux_init(void) {
	...
	for_each_matching_node(pp, ipq806x_gpiomux_ids) {
		for_each_child_of_node(pp, child) {
			of_property_read_u32(child, "pin", &pin);
			of_property_read_u32_array(child, "active",  active, 4);
			of_property_read_u32_array(child, "suspended",  suspended, 4);

			msm_dt_cryptid_gpiomux[npins].gpio = pin;
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_ACTIVE].func = active[0];
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_ACTIVE].drv = active[1];
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_ACTIVE].pull = active[2];
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_ACTIVE].dir = active[3];
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_SUSPENDED].func = suspended[0];
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_SUSPENDED].drv = suspended[1];
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_SUSPENDED].pull = suspended[2];
			msm_dt_cryptid_gpiomux[npins].settings[GPIOMUX_SUSPENDED].dir = suspended[3];
			npins++;
		}
	}
	msm_gpiomux_dt_install(msm_dt_cryptid_gpiomux, npins);
}
