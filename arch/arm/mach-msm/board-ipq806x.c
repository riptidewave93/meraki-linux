...
void ipq806x_pcie_pdata_fixup(void)
{
	...
	if (machine_is_ipq806x_ap160()) {
		if (of_machine_is_compatible("meraki,yowie")) {
			msm_pcie_platform_data[0].force_gen1 = 0;
			msm_pcie_platform_data[1].force_gen1 = 1;
		} else {
			msm_pcie_platform_data[1].force_gen1 = 0;
			msm_pcie_platform_data[2].force_gen1 = 1;
		}
	}
...
static struct i2c_gpio_platform_data ipq806x_i2c_gpio_gsbi2 = {
	.sda_pin = 24,
	.scl_pin = 25,
};
...
#define GSBI_UART_FLOW_CODE	0x40
#define UARTDM_FLOW_ON		0xC0
#define MSM_GSBI1_PHYS		0x12440000
#define MSM_GSBI4_PHYS		0x16300000
#define MSM_UART1_DM_PHYS	0x12450000
static void __init ipq806x_i2c_init(void)
{
	...
	wmb();
	iounmap(gsbi_mem);
	ipq806x_i2c_qup_gsbi1_pdata.use_gsbi_shared_mode = 1; //TODO dual mode sasquatch
	...
	ipq806x_device_gpio_i2c_gsbi2.dev.platform_data =
					&ipq806x_i2c_gpio_gsbi2;
}

static void __init ipq806x_ble_uart_init(void)
{
	void __iomem *gsbi_mem;
	uint32_t uart_config;

	msm_clock_init(&ipq806x_gsbi1_uart_clks);

	// Configure GSBI block
	gsbi_mem = ioremap_nocache(MSM_GSBI1_PHYS, 4);
	uart_config = readl_relaxed(gsbi_mem);
	writel_relaxed(GSBI_UART_FLOW_CODE, gsbi_mem);
	wmb();
	iounmap(gsbi_mem);

	// Configure UART DM block
	gsbi_mem = ioremap_nocache(MSM_UART1_DM_PHYS, 4);
	uart_config = readl_relaxed(gsbi_mem);
	uart_config |= UARTDM_FLOW_ON;
	writel_relaxed(uart_config, gsbi_mem);
	wmb();
	iounmap(gsbi_mem);
}
...
static void __init ipq806x_common_init(void)
{
	...
	if (machine_is_ipq806x_ap160()) {
	       struct ramoops_platform_data *ramoops_pdata = (struct ramoops_platform_data*)ramoops_device.dev.platform_data;
...
static struct of_device_id ipq806x_gmac_ids[] __initdata = {
	{ .compatible = "ipq806x,nss-gmac", },
	{}
};

static void __init ipq806x_dt_gmac_init(void)
{
	struct msm_nss_gmac_platform_data *pdata;
	struct device_node *pp;
	struct platform_device *nss_gmac_ptr;
	int nss_gmac;

	mdiobus_register_board_info(ipq806x_ap160_mdio_info, IPQ806X_MDIO_BUS_MAX);
	for_each_matching_node(pp, ipq806x_gmac_ids) {
		of_property_read_u32(pp, "gmac", &nss_gmac);
		switch(nss_gmac) {
			case 0:
				nss_gmac_ptr = &nss_gmac_0;
				break;
			case 1:
				nss_gmac_ptr = &nss_gmac_1;
				break;
			case 2:
				nss_gmac_ptr = &nss_gmac_2;
				break;
			case 3:
				nss_gmac_ptr = &nss_gmac_3;
				break;
			default:
				nss_gmac_ptr = NULL;
		};
		if(nss_gmac_ptr) {
			pdata = (struct msm_nss_gmac_platform_data *)nss_gmac_ptr->dev.platform_data;
			of_property_read_u32(pp, "phy-addr", &pdata->phy_mdio_addr);
			pdata->rgmii_delay = 0;
			pdata->socver = socinfo_get_version();
			of_property_read_u32(pp, "poll_required", &pdata->poll_required);
			of_property_read_u32(pp, "phy_mii_type", &pdata->phy_mii_type);
			of_property_read_u32(pp, "forced_speed", &pdata->forced_speed);
			of_property_read_u32(pp, "forced_duplex", &pdata->forced_duplex);
			of_property_read_u32(pp, "mmds_mask", &pdata->mmds_mask);
			platform_device_register(nss_gmac_ptr);
		}
	}
}
...
static void nss_gmac_init(void)
{
	...
	if (of_machine_is_compatible("meraki,cryptid-aq105")) {
		// BUS 0 8033 ARQ105
		...
		// BUS 1 8511
		mdata = (struct mdio_gpio_platform_data *)ipq806x_mdio_device2.dev.platform_data;
		mdata->mdc = 1;
		mdata->mdio = 0;
		mdata->phy_mask = 0;
...
#ifdef CONFIG_OF
static const char *cryptid_boards_compat[] __initdata = {
	"meraki,yowie",
	"meraki,bigfoot",
	"meraki,sasquatch",
	"meraki,wookie",
...
MACHINE_START(IPQ806X_AP148, "Qualcomm Atheros AP148 reference board")
	...
	.restart = msm_restart,
#ifdef CONFIG_OF
	.dt_compat = cryptid_boards_compat,
#endif
MACHINE_END
