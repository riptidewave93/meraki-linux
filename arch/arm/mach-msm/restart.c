...
#endif
	if (in_panic) {
		__raw_writel(REASON_PANIC, RESTART_REASON_ADDR);
	} else {
		__raw_writel(REASON_RESTART, RESTART_REASON_ADDR);
	}

	if (of_machine_is_compatible("meraki,yowie")) {
		if( gpio_request(6, "phy-rst6") == 0) {
			gpio_direction_output(6, 0);
		}
	}

	if (of_machine_is_compatible("meraki,bigfoot") ||
       of_machine_is_compatible("meraki,sasquatch") ||
       of_machine_is_compatible("meraki,wookie")) {
		if( gpio_request(6, "phy-rst6") == 0) {
			gpio_direction_output(6, 0);
		}

		if( gpio_request(7, "phy-rst7") == 0) {
