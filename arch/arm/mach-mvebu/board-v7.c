...
static const char * const kelpie_dt_compat[] __initconst = {
	"meraki,kelpie",
	NULL,
};

static int kelpie_restart_notify(struct notifier_block *this,
				 unsigned long mode, void *cmd)
{
	mvebu_restart(reboot_mode, cmd);
	return NOTIFY_DONE;
}

static struct notifier_block restart_handler;
static void __init kelpie_early_init(void)
{
	int ret;

	restart_handler.notifier_call = kelpie_restart_notify;
	restart_handler.priority = 128;
	ret = register_restart_handler(&restart_handler);
	if (ret) {
		printk(KERN_ERR "Kelpie cannot register restart handler\n");
		return;
	}
}

DT_MACHINE_START(MERAKI_KELPIE_DT, "Meraki Kelpie (Device Tree)")
	.l2c_aux_val	= 0,
	.l2c_aux_mask	= ~0,
/*
 * The following field (.smp) is still needed to ensure backward
 ...
 */
	.smp		= smp_ops(armada_xp_smp_ops),
	.init_machine	= mvebu_dt_init,
	.init_early	= kelpie_early_init,
	.init_irq       = msys_irqchip_init,
	.restart	= NULL,
	.reserve        = mvebu_memblock_reserve,
	.dt_compat	= kelpie_dt_compat,
MACHINE_END
