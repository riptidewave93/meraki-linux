...
#if defined(CONFIG_OF)
static int __init early_init_dt_get_amb_reg(unsigned long node, const char *reg_name,
                                            u32 *base, u32 *size, u32 *flags)
{
	unsigned long l;
	__be32 *reg, *endp;

	reg = of_get_flat_dt_prop(node, reg_name, &l);
	if (reg == NULL)
		return 0;
	...
	*base = be32_to_cpu(reg[0]);
	if (size)  *size = be32_to_cpu(reg[1]);
	if (flags) *flags = be32_to_cpu(reg[2]);
	return 1;
}

/* This function is called for every node in the device tree. We are
   looking for a node named ambarella. If one is found, then extract
   the BST, BSB and DSP memory pointers. These values were originally
   passed by the bootloader via atags. The early linux decompressor
   copied these values from atags and put them in the device tree
 */
extern unsigned int system_rev;
int __init early_init_dt_scan_ambarella(unsigned long node, const char *uname,
				     int depth, void *data)
{
   u32 base, size, flags;

	pr_debug("\nsearch \"ambarella\", depth: %d, uname: %s\n\n", depth, uname);

	if (depth != 1 || (strcmp(uname, "ambarella") != 0 ))
		return 0;

	if (early_init_dt_get_amb_reg(node, "bsb-reg", &base, &size, NULL)) {
	   bsb_mem_check(base, size);
   }

	if (early_init_dt_get_amb_reg(node, "dsp-reg", &base, &size, NULL)) {
	   dsp_mem_check(base, size);
   }

	if (early_init_dt_get_amb_reg(node, "bst-reg", &base, &size, NULL)) {
	   ambarella_bst_info.physaddr = base;
	   ambarella_bst_info.size = size;
   }

	if (early_init_dt_get_amb_reg(node, "board-id", &base, NULL, NULL)) {
	   system_rev = base;
   }

#if defined(CONFIG_PLAT_AMBARELLA_SUPPORT_HAL)
	if (early_init_dt_get_amb_reg(node, "hal-reg", &base, &size, &flags)) {
	    hal_mem_check(base, size, flags);
   }
#endif
