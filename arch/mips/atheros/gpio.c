...
int ath_gpio_in_val(int);

static unsigned long irq_bothedge[BITS_TO_LONGS(ATH_GPIO_IRQ_COUNT)];

int
ath_gpio_set_irq_type(unsigned int irq, unsigned int flow_type)
{
	int gpio = irq - ATH_GPIO_IRQ_BASE;

	clear_bit(gpio, irq_bothedge);
	...
	switch (flow_type & IRQ_TYPE_SENSE_MASK) {
		...
		case IRQ_TYPE_LEVEL_LOW:
			ath_reg_rmw_set(ATH_GPIO_INT_TYPE, (1 << gpio));
			ath_reg_rmw_clear(ATH_GPIO_INT_POLARITY, (1 << gpio));
			break;
		case IRQ_TYPE_EDGE_RISING:
			ath_reg_rmw_clear(ATH_GPIO_INT_TYPE, (1 << gpio));
			ath_reg_rmw_set(ATH_GPIO_INT_POLARITY, (1 << gpio));
			break;
		...
		case IRQ_TYPE_EDGE_BOTH:
			set_bit(gpio, irq_bothedge);
			ath_reg_rmw_clear(ATH_GPIO_INT_TYPE, (1 << gpio));
			if (ath_gpio_in_val(gpio))
				ath_reg_rmw_clear(ATH_GPIO_INT_POLARITY, (1 << gpio));
			else
				ath_reg_rmw_set(ATH_GPIO_INT_POLARITY, (1 << gpio));
			break;
		default:
			printk("failed to set irq %d to flow_type 0x%x\n", gpio, flow_type);
			return -EINVAL;
	}
...
int ath_gpio_config_output(int gpio, int val)
{
...
#endif
    ath_gpio_out_val(gpio, val);
    return 0;
}
EXPORT_SYMBOL(ath_gpio_config_output);

int ath_gpio_config_input(int gpio)
{
#ifdef CONFIG_SOC_AR934x
	ath_reg_rmw_set(ATH_GPIO_OE, (1 << gpio));
#else
...
{
	ath_reg_rmw_set(ATH_GPIO_INT_ENABLE,
...
{
	ath_reg_rmw_clear(ATH_GPIO_INT_ENABLE,
				(1 << (irq - ATH_GPIO_IRQ_BASE)));
...
{
	int gpio = irq - ATH_GPIO_IRQ_BASE;
	ath_reg_rmw_clear(ATH_GPIO_INT_MASK, (1 << gpio));

	//swap the polarity of the interrupt
	if (test_bit(gpio, irq_bothedge))
		ath_gpio_set_irq_type(irq, IRQ_TYPE_EDGE_BOTH);
}
...
static int gpiolib_direction_input(struct gpio_chip *chip, unsigned gpio)
{
        return ath_gpio_config_input(gpio);
}

static int gpiolib_direction_output(struct gpio_chip *chip, unsigned gpio, int v)
{
        return ath_gpio_config_output(gpio, v);
}

static int gpiolib_get_value(struct gpio_chip *chip, unsigned gpio)
{
        return ath_gpio_in_val(gpio);
}

static void gpiolib_set_value(struct gpio_chip *chip, unsigned gpio, int v)
{
        ath_gpio_out_val(gpio, v);
}

static int gpiolib_to_irq(struct gpio_chip *chip, unsigned gpio)
{
        return ATH_GPIO_IRQn(gpio);
}

static struct gpio_chip atheros_gpio = {
	.label = "atheros-gpio",
    ...
    .to_irq = gpiolib_to_irq,
	.base = ATHEROS_GPIO_BASE,
	.ngpio = ATHEROS_NUM_GPIOS,
	.can_sleep = 0,
};

static int __init gpiolib_init(void)
{
    return gpiochip_add(&atheros_gpio);
}
...
struct irq_chip /* hw_interrupt_type */ ath_gpio_intr_controller = {
	...
	.set_type = ath_gpio_set_irq_type,
};
...
static int create_simple_config_led_proc_entry(void)
{
	...
	/* configure gpio as outputs and switch it off*/
	ath_gpio_config_output(WPS_LED_GPIO, WPS_LED_OFF);
...
#if !defined(CONFIG_I2S) && defined(AP_USB_LED_GPIO)
	ret = ath_gpio_config_output(AP_USB_LED_GPIO, USB_LED_OFF);
#endif
