...
nand_status_loop_check(void)
{
    ...
    while (ath_reg_rd(ATH_NF_STATUS) != 0xff && count++ <= TRY_MAX)
        udelay(1);
...
nand_read_id(void)
{
    ath_reg_wr(ATH_NF_DMA_ADDR,
               ((unsigned int)nand_id) & ~KSEG1);       // DMA Start Addr XXX assuming we're in Kseg1
    ath_reg_wr(ATH_NF_ADDR0_0, 0x0);                    // ADDR0_0 Reg Settings
    ath_reg_wr(ATH_NF_ADDR0_1, 0x0);                    // ADDR0_1 Reg Settings
    ath_reg_wr(ATH_NF_DMA_COUNT, sizeof(*nand_id));     // DMA count
    ath_reg_wr(ATH_NF_PG_SIZE, 0x8);                    // Custom Page Size
    ath_reg_wr(ATH_NF_DMA_CTRL, 0xcc);                  // DMA Control Reg
...
ath_nand_status(uint32_t* rddata)
{
    ...
    count = 0;
    while (!(ath_reg_rd(ATH_NF_DMA_CTRL) & 1) && count++ <= TRY_MAX)
        udelay(1);
...
nand_read(unsigned int row, unsigned int count, uint8_t *buf, void(*cb)(struct bch_crc_params *), struct bch_crc_params* param)
{
    ...
    while (retries++ <= TRY_MAX) {
        ...
        ath_reg_wr(ATH_NF_ADDR0_0, row << 16);

        // ADDR0_1 Reg Settings
        ath_reg_wr(ATH_NF_ADDR0_1, row >> 16);

        // DMA Start Addr
        ath_reg_wr(ATH_NF_DMA_ADDR, ((unsigned int)buf));
        ...
        if (ath_nand_status(&rstatus)) {
            printf("E");
            nand_flash_init();
        } else if ((rstatus & ATH_NF_RD_STATUS_MASK) == ATH_NF_STATUS_OK) {
            break;
        }
    }
...
nand_load_bk_skipping(int addr, void(**kernel_entry)(int, char **, int))
{
    ...
    do {
        ...
        lr = nand_load_bk(addr, kernel_entry);
        {
...
nand_load_fw(void)
{
    ...
    lr1 = nand_load_bk_skipping(BOOTKERNEL1_PAGE_ADDR, &kernel_entry);
    if (lr1 != BK_OK) {
        enum bk_load_result lr2;
        printf("bootkernel 1 failed!\n");
        lr2 = nand_load_bk_skipping(BOOTKERNEL2_PAGE_ADDR, &kernel_entry);
        if (lr2 != BK_OK) {
