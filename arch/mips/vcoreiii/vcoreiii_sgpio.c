...
static int __devinit vcoreiii_sgpio_probe(struct platform_device *pdev)
{
...
static int __devexit vcoreiii_sgpio_remove(struct platform_device *pdev)
{
	struct sgpio_gpio_state *sgpio = platform_get_drvdata(pdev);
	int ret;

	ret = gpiochip_remove(&sgpio->gpio_chip);
	if (ret) {
		return ret;
	}
...
static struct platform_driver vcoreiii_sgpio_driver = {
	...
	.remove		= __devexit_p(vcoreiii_sgpio_remove),
	.driver		= {
		.name	= "vcoreiii-sgpio",
		.owner	= THIS_MODULE,
	},
