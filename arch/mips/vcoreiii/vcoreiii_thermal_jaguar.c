...
struct vcoreiii_tdev {
	...
	struct proc_dir_entry *procent_warm;
	int temp_warm;
};
...
{
	struct vcoreiii_tdev *vctdev = (struct vcoreiii_tdev *)
		(PDE(file->f_dentry->d_inode)->data);
					    ...
					    volatile void __iomem *data_reg)
{
	...
	tdev = thermal_zone_device_register(name, 1, &vcoreiii_tdevs[which],
					    &vcoreiii_thermal_zone_ops,
					    0, 0, 0, 10000);
	if (IS_ERR(tdev)) {
	...
	if (vcoreiii_thermal_dir) {
		...
		if (!vcoreiii_tdevs[which].procdir) {
			...
			vcoreiii_tdevs[which].procent_warm =
				proc_create_data("temp_warm",
						 S_IFREG | S_IRUGO | S_IWUSR,
						 vcoreiii_tdevs[which].procdir,
						 &vcoreiii_thermal_proc_ops,
						 &vcoreiii_tdevs[which]);
			if (!vcoreiii_tdevs[which].procent_warm) {
				printk(KERN_WARNING
				       "Unable to create "
...
static void __exit vcoreiii_thermal_exit(void)
{
	...
	for (i = 0; i < TEMP_SENSORS; i++) {
		if (vcoreiii_tdevs[i].procdir) {
			if (vcoreiii_tdevs[i].procent_warm) {
				remove_proc_entry(vcoreiii_tdevs[i].
						  procent_warm->name,
						  vcoreiii_tdevs[i].
						  procent_warm->parent);
				vcoreiii_tdevs[i].procent_warm = NULL;
			}
			remove_proc_entry(vcoreiii_tdevs[i].procdir->name,
					  vcoreiii_tdevs[i].procdir->parent);
			vcoreiii_tdevs[i].procdir = NULL;
		}
	...
	if (vcoreiii_thermal_dir) {
		remove_proc_entry(vcoreiii_thermal_dir->name,
				  vcoreiii_thermal_dir->parent);
