...
#if defined(CONFIG_APM862xx) || defined(CONFIG_APM866xx)
 ...
 * Bit (1-23)  - Reserved: Reserved for new features in SlimPro firmware
 * Bit (24) - CodeSwap: CodeSwap support in firmware
 * Bit (25) - NATOffload: NAT offload support in firmware
 * Bit (26) - SNMP: LRO support in firmware
...
/** NAT Offload Feature Bit mask */
#define IPP_NATOFFLOAD_FW_MASK		0x00000040
/** NAT Offload Feature Bit shift */
#define IPP_NATOFFLOAD_FW_SHIFT		6
...
/** Power Management Feature Bit shit */
...
#define IPP_NETDATA_NAT_OFFLOAD_TYPE			5
...
/* LRO Command Type */
#define IPP_LRO_TIMEOUT			0
#define IPP_LRO_BYTECNT			1
#define IPP_LRO_Q0			2	/* Domain 0 */
#define IPP_LRO_Q1			3	/* Domain 1 */
...
/* IPP LRO Command for setting timeout */
struct ipp_lro_timeout {
	unsigned int cmd;
	unsigned int timeout;
};

/* IPP LRO Command for setting byte count */
struct ipp_lro_bytecnt {
	unsigned int cmd;
	unsigned int bytecnt;
};

/* IPP LRO Command for setting Q info */
struct ipp_lro_q {
	unsigned int cmd;
#if defined(__PPC__)	
	unsigned int port : 8;
	unsigned int no_cpu : 8;
	unsigned int ppc0_wq : 8;
	unsigned int ppc1_wq : 8;
	
	unsigned int tcpipmac_fq : 8;
	unsigned int hdr_fq : 8;
	unsigned int hdr_fqpbn : 8;
	unsigned int data_fq : 8;
	
	unsigned int dma_wq : 8;
	unsigned int dma_cq : 8;
	unsigned int slimpro_wq : 8;
	unsigned int rsvd1 : 8;
	
	unsigned int data_bufsiz;
	unsigned int hdr_bufsiz;
	
	unsigned int ppc_domains : 8;
	unsigned int allow_qfree : 8;
	unsigned int coherent_ddr : 8;
	unsigned int resvd2 : 8;
#else
	unsigned int ppc1_wq : 8;
	unsigned int ppc0_wq : 8;
	unsigned int no_cpu : 8;
