 ...
 *          Robert Ghilduta <robg@meraki.com>
 *
...
/**
 * topaz_get_pixel_format: return the Area Descriptor for a given pixel depth
 *
 * The Area Descriptor is a 32-bit value that determine which bits in each
 * pixel are to be used for each color.
 */
static u32 topaz_get_pixel_format(enum fsl_diu_monitor_port port,
...
/**
 * topaz_set_gamma_table: update the gamma table, if necessary
 *
 * On some boards, the gamma table for some ports may need to be modified.
 * This is not the case on the P1022DS, so we do nothing.
*/
static void topaz_set_gamma_table(enum fsl_diu_monitor_port port,
...
/**
 * topaz_set_monitor_port: switch the output to a different monitor port
 */
static void topaz_set_monitor_port(enum fsl_diu_monitor_port port)
{
...
/**
 * topaz_set_pixel_clock: program the DIU's clock
 *
 * @pixclock: the wavelength, in picoseconds, of the clock
 */
void topaz_set_pixel_clock(unsigned int pixclock)
{
...
/**
 * topaz_valid_monitor_port: set the monitor port for sysfs
 */
enum fsl_diu_monitor_port
topaz_valid_monitor_port(enum fsl_diu_monitor_port port)
{
	switch (port) {
	case FSL_DIU_PORT_DVI:
	case FSL_DIU_PORT_LVDS:
...
static struct gpio_led topaz_led_pins[] = {
        {
            .name = "power:orange",
            .gpio = 232,
...
static struct gpio_led_platform_data topaz_led_data = {
        .leds = topaz_led_pins,
        .num_leds = ARRAY_SIZE(topaz_led_pins),
};

static struct platform_device topaz_leds = {
        .name = "leds-gpio",
        .id = -1,
        .dev = {
            .platform_data = &topaz_led_data,
        }
};
 ...
 * global variable.  Later on, topaz_setup_arch() will use that variable
...
#if defined(CONFIG_FB_FSL_DIU) || defined(CONFIG_FB_FSL_DIU_MODULE)
	diu_ops.get_pixel_format	= topaz_get_pixel_format;
	diu_ops.set_gamma_table		= topaz_set_gamma_table;
	diu_ops.set_monitor_port	= topaz_set_monitor_port;
	diu_ops.set_pixel_clock		= topaz_set_pixel_clock;
	diu_ops.valid_monitor_port	= topaz_valid_monitor_port;
...
machine_device_initcall(topaz, mpc85xx_common_publish_devices);
