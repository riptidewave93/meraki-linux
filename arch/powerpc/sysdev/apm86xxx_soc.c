...
static int ahb_suspend(struct sys_device *dev, pm_message_t state)
{
	if (ahbc_base && suspending_to_deepsleep()) {
		int i;
		u32 __iomem *ahb_reg = (u32 __iomem *)(ahbc_base + 0x2000);

		for (i = 0; i < sizeof(ahb_in_map)/sizeof(u32); i++)
			ahb_in_map[i] = in_be32(&ahb_reg[57 + i]);

#if !defined(CONFIG_APM867xx)
		/* Clear ahbc_usb_enable flag so that of_driver resume
		 * will enable AHBC USB[port] clocks */
...
static int ahb_resume(struct sys_device *dev)
{
	if (ahbc_base && resumed_from_deepsleep()) {
		int i;
		u32 __iomem *ahb_reg = (u32 __iomem *)(ahbc_base + 0x2000);

		for (i = 0; i < sizeof(ahb_in_map)/sizeof(u32); i++)
			out_be32(&ahb_reg[57 + i], ahb_in_map[i]);
	}

	return 0;
}

static struct sysdev_class ahb_pm_sysclass = {
	.name = "ahb_pm",
	.suspend = ahb_suspend,
	.resume = ahb_resume,
};

static struct sys_device ahb_pm_device = {
	.cls = &ahb_pm_sysclass
};

static int __init ahb_pm_init(void)
{
	int err;

	err = sysdev_class_register(&ahb_pm_sysclass);
	if (err == 0)
		err = sysdev_register(&ahb_pm_device);

	return err;
}
device_initcall(ahb_pm_init);
#endif
