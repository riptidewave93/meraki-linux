...
static int apm_qm_driver_ioctl(struct inode *inode, struct file *file,
...
int apm_qm_config_set(struct of_device *ofdev)
{
	int rc = 0;
	int inum = 0, i = 0, j = 0;
...
struct file_operations apm_qm_driver_fops = {
	...
	.ioctl = apm_qm_driver_ioctl,
};

static int __devinit apm_qm_probe(struct of_device *ofdev,
...
static int __devexit apm_qm_remove(struct of_device *dev)
{
...
#ifdef CONFIG_PM
static int apm_qm_of_suspend(struct of_device* ofdev, pm_message_t state)
{
	int i;
	int rc = 0;
	...
	if ((!suspending_to_deepsleep()) || (!is_smp && (mfspr(SPRN_PIR))))
		return rc;

	printk("Saving QM states ...\n");
	/* save QM states for all queues */
	for (i = 1; i < DSLEEP_MAX_QUEUES_TOSAVE; i++) {
		if ((rc = apm_qm_raw_qstate_rd(IP_BLK_QM, i, &q_raw_states[i]) != 0)) {
			printk("Error reading QM queue: %d\n", i);
			return -1;
		}
	}

	printk("Disable QM QPcore ...\n");
	/* Disable QM QPcore */
...
static int apm_qm_of_resume(struct of_device* ofdev)
{
	int i;
	u32 val = 0;
	int is_fp = 0;
	...
	printk("Restoring QM states ...\n");
	/* FIXME - Restore PBN statuts known on core1 only for AMP */
	if (!is_smp && mfspr(SPRN_PIR)) {
		for (i = 125; i < DSLEEP_MAX_QUEUES_TOSAVE; i++) {
	...
	/* restore QM states of all queues */
	for (i = 1; i < DSLEEP_MAX_QUEUES_TOSAVE; i++) {
		...
		if (pbn_valid[i] == 1) {
			...
			is_fp = ((qstate->q_type == FREE_POOL) ? 1 : 0);
			is_vq = ((qstate->q_type == V_QUEUE) ? 1 : 0);
			if (qstate->slave_id == 5 && (qstate->pbn == 0x21 ||
						      qstate->pbn == 0x28 ))  {
				apm_qm_pb_overwrite(qstate->ip_blk,
						   qstate->slave_id, 
						   qstate->pbn,
...
static struct of_platform_driver apm_qm_driver = {
	.name		= "mb-qmtm",
	.match_table	= apm_qm_match,
	.probe		= apm_qm_probe,
...
{
	return of_register_platform_driver(&apm_qm_driver);
}

void __exit apm_qm_driver_cleanup(void)
{
	of_unregister_platform_driver(&apm_qm_driver);
