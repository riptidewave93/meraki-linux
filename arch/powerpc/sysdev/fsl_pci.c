				  ...
				  struct resource *rsrc)
{
	...
	/* SKYROS AP1600 specific hack for now, eventually move this to DTS */
#if 0
	printk("**********************************************\n");
	u32 x;
	x = 0x0004401A;
	out_be32(&pci->pow[0].powar, x);
	x = 0x8004401A;
	out_be32(&pci->pow[1].powar, x);
	x = 0xa0fff01b;
	out_be32(&pci->piw[0].piwar, x);
	x = 0xa0fff01b;
	out_be32(&pci->piw[2].piwar, x);

#if 0
	for(x=0; x<5; x++) {
		printk("POTAR0 = 0x%x\n", in_be32(&pci->pow[x].potar));
		printk("POTEAR0= 0x%x\n", in_be32(&pci->pow[x].potear));
		printk("POWBAR0= 0x%x\n", in_be32(&pci->pow[x].powbar));
		printk("OWAR0  = 0x%x\n", in_be32(&pci->pow[x].powar));
		printk("\n");
	}
	for(x=0; x<4; x++) {
		printk("PITAR0 = 0x%x\n", in_be32(&pci->piw[x].pitar));
		printk("PIWBAR0= 0x%x\n", in_be32(&pci->piw[x].piwbar));
		printk("IWAR0  = 0x%x\n", in_be32(&pci->piw[x].piwar));
		printk("\n");
	}
#endif
#endif
