...
static int __devinit ipp_probe(struct of_device *ofdev,
				const struct of_device_id *match)
{
	...
	cpu = (num_online_cpus() > 1) ? DEFAULT_SMP_IPPMSG_CPU :
				hard_smp_processor_id();
	ipp->ofdev = ofdev;
        dev_set_drvdata(&ofdev->dev, ipp);

	err = add_ipp_sysfs(ofdev->dev.driver);
	if (err)
		goto fail;
...
fail:
	dev_err(&ofdev->dev, "iPP Probe Failed\n");
	...
	if (ipp->regs)
		iounmap(ipp->regs);
	remove_ipp_sysfs(ofdev->dev.driver);
	return err;
}
				...
				int len)
{
	...
	phys_addr = dma_map_single(&ipp_data.ofdev->dev, addr_buf,
				len, DMA_TO_DEVICE);
	if (!phys_addr) {
		dev_info(&ipp_data.ofdev->dev,
				...
				void *buf, int len)
{
	...
	phys_addr = dma_map_single(&ipp_data.ofdev->dev,
				(void *)addr_msg_buf,
				len, DMA_TO_DEVICE);
	if (!phys_addr) {
		dev_info(&ipp_data.ofdev->dev,
...
static struct of_platform_driver ipp_driver = {
	.driver = {
		.name	= "ipp",
	},
	.match_table = ipp_match,
	.probe = ipp_probe,
};
...
static int __init ipp_init(void)
{
	...
	return of_register_platform_driver(&ipp_driver);
error:
