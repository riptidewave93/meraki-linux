...
__attribute__((weak))
int of_ahci_phy_chk(struct of_device *ofdev)
{
...
__attribute__((weak))
int of_ahci_phy_cleanup(struct of_device *ofdev)
{
...
__attribute__((weak))
int of_ahci_set_port_ops(struct of_device *ofdev, struct ata_port_info *pip)
{
...
#ifdef CONFIG_PM
__attribute__((weak))
int of_ahci_phy_suspend(struct of_device* ofdev, pm_message_t state)
{
	return 0;
}

__attribute__((weak))
int of_ahci_phy_resume(struct of_device* ofdev)
{
	return 0;
}
#endif /* CONFIG_PM */
...
static int __devinit of_ahci_probe(struct of_device *ofdev,
			        const struct of_device_id *match)
{
	...
	retval = of_ahci_set_port_ops(ofdev, &pi);
	if (retval != 0) {
...
static int __devexit of_ahci_remove(struct of_device *ofdev)
{
	struct device *dev = &ofdev->dev;
	struct ata_host *host = dev_get_drvdata(dev);
	struct ahci_host_priv *hpriv = host->private_data;

	of_ahci_phy_cleanup(ofdev);
...
#ifdef CONFIG_PM
static int of_ahci_suspend(struct of_device* ofdev, pm_message_t state)
{
	return of_ahci_phy_suspend(ofdev, state);
}

static int of_ahci_resume(struct of_device* ofdev)
{
	return of_ahci_phy_resume(ofdev);
}
#endif
...
static struct of_platform_driver of_ahci_driver = {
	.name = "ahci-sata-of",
	.match_table = of_ahci_id_table,
	.probe = of_ahci_probe,
	.remove = __devexit_p(of_ahci_remove),
#ifdef CONFIG_PM
	.suspend = of_ahci_suspend,
	.resume = of_ahci_resume,
#endif
	.driver = {
		.name = DRV_NAME,
		.owner = THIS_MODULE,
...
{
	return of_register_platform_driver(&of_ahci_driver);
}

static void __exit of_ahci_exit(void)
{
	of_unregister_platform_driver(&of_ahci_driver);
}
