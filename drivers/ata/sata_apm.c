...
int of_ahci_phy_chk(struct of_device *ofdev)
{
	...
	num_pcs_entry = (u32 *) of_get_property(ofdev->node, "phy", NULL);
	if (num_pcs_entry == NULL || *num_pcs_entry == 0) {
		dev_err(dev, "no phy entry in DTS\n");
		return 0;
	}
	
	num_pcs_act_entry = (u32 *) of_get_property(ofdev->node, "phy-act", NULL);
	...
	val_ptr = (u32 *) of_get_property(ofdev->node, "defaultspd", NULL);
	if (val_ptr != NULL) {
	...
	if (num_pcs_entry && *num_pcs_entry) {
		...
		apm_pdata->apm_sata_diag = of_iomap(ofdev->node, 1);
		...
		for (i = 0; i < apm_pdata->num_pcs; i++) {
			...
			apm_pdata->apm_sata_pcs[i] = of_iomap(ofdev->node,
								(i + 2));
...
#ifdef CONFIG_SYSFS
	add_sata_sysfs(ofdev->dev.driver);
#endif
...
void of_ahci_phy_cleanup(struct of_device *ofdev)
{
	int i;
	struct device *dev = &ofdev->dev;
...
int of_ahci_phy_suspend(struct of_device* ofdev, pm_message_t state)
{
	...
	for (i = 0; i < apm_pdata->num_pcs; i++) {
		...
		if (!(apm_pdata->phy_active & (1 << i)))
			continue;		
		ahci_ops.port_suspend(ap, state);
	}
#endif
...
__outer_retry:		
	 ...
	 */ 
	start_jiffies = jiffies;
	val = ata_wait_register(port_mmio + PORT_SIG, 0xFF, 0xFF, 100, 500);
	if (val == 0xFFFFFFFF) {
		if (--outer_retry > 0) {
