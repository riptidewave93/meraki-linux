...
void apm_lsec_sg2buf(struct scatterlist *src, int nbytes, char *saddr)
{
	...
	for (i = 0; nbytes > 0; i++) {
		spage = kmap_atomic(sg_page(&src[i]), KM_SOFTIRQ0);
		vaddr = spage + src[i].offset;
		memcpy(saddr, vaddr, src[i].length);
		kunmap_atomic(spage, KM_SOFTIRQ0);
		nbytes -= src[i].length;
		saddr  += src[i].length;
	}
}
			     ...
			     struct sec_tkn_ctx *tkn)
{
	...
	spage = kmap_atomic(sg_page(src), KM_SOFTIRQ0);
			 ...
			 struct crypto_async_request *req)
{
	...
	len = dst[0].length;
	dpage = kmap_atomic(sg_page(&dst[0]), KM_SOFTIRQ0);
	...
	dest    += len;
	for (i = 1; nbytes > 0; i++) {
		dpage = kmap_atomic(sg_page(&dst[i]), KM_SOFTIRQ0);
...
#else
#define apm_sec_suspend NULL
#define apm_sec_resume NULL
#endif /* CONFIG_PM */

extern unsigned int meraki_crypto_memory_allocated;

static int __devinit apm_sec_probe(struct of_device *ofdev,
...
probe_no_tm:	
	...
	dev_set_drvdata(&ofdev->dev, &sec_ctx);
...
static int __devexit apm_sec_remove(struct of_device *ofdev)
{
	struct apm_sec_ctx * ctx = dev_get_drvdata(&ofdev->dev);
...
static int apm_sec_shutdown(struct of_device *ofdev)
{
	int rc = 0;
#ifndef CONFIG_SMP
	return rc;
...
static struct of_platform_driver apm_sec_driver = {
	.name		= "apm-crypto",
	.match_table	= apm_sec_match,
	.probe		= apm_sec_probe,
	.remove		= apm_sec_remove,
#if defined(CONFIG_PM)
	.suspend	= apm_sec_suspend,
	.resume 	= apm_sec_resume,
#endif
	.shutdown 	= apm_sec_shutdown,
};

static int __init apm_sec_init(void)
{
	return of_register_platform_driver(&apm_sec_driver);
}

static void __exit apm_sec_exit(void)
{
	of_unregister_platform_driver(&apm_sec_driver);
}
