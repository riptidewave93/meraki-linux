...
static int apm_sec_encrypt_aes_ccm(struct aead_request *req)
{
	...
	in_tkn = TKN_CTX_INPUT_TKN(rctx->tkn);
	if (req->assoclen) {
		adata = kmap_atomic(sg_page(&req->assoc[0]), KM_SOFTIRQ0) +
						req->assoc[0].offset;
	}
	APMSEC_SATKNLOG("alg %s tkn 0x%p outtkn 0x%p intkn 0x%p",
	...
	if (adata)
		kunmap_atomic(adata, KM_SOFTIRQ0);
					 ...
					 struct aead_request *req)
{
	...
	page = kmap_atomic(sg_page(&req->src[0]), KM_SOFTIRQ0);
	...
	if (*SA_SPI(session->sa->sa_ptr) == 0) {
		...
		page = kmap_atomic(sg_page(req->assoc), KM_SOFTIRQ0);
	...
	if (apm_sec_sg_scattered(req->cryptlen, req->dst)) {
		...
		dpage = kmap_atomic(sg_page(&req->dst[0]), KM_SOFTIRQ0);
...
static int apm_sec_encrypt_ah_tunnel(struct aead_request *req)
{
	...
	page = kmap_atomic(sg_page(&req->dst[0]), KM_SOFTIRQ0);
					...
					unsigned int aes_mode)
{
	...
	if (req->assoclen)
		adata = kmap_atomic(sg_page(req->assoc), KM_SOFTIRQ0);
	in_tkn = TKN_CTX_INPUT_TKN(rctx->tkn);
	if (new_tkn) {
