...
#define QCRYPTO_RFC3686_NONCE_LEN	4
#define QCRYPTO_RFC3686_IV_LEN		8
...
struct qcrypto_cipher_ctx {
	...
	u8 rfc3686_nonce[QCRYPTO_RFC3686_NONCE_LEN];
...
struct qcrypto_cipher_req_ctx {
	...
	u8 rfc3686_iv[QCRYPTO_MAX_IV_LENGTH + QCRYPTO_RFC3686_NONCE_LEN];
	unsigned int ivsize;
	int  aead;
...
static int _qcrypto_setkey_rfc3686(struct crypto_ablkcipher *cipher, const u8 *key,
		unsigned int len)
{
	struct crypto_tfm *tfm = crypto_ablkcipher_tfm(cipher);
	struct qcrypto_cipher_ctx *ctx = crypto_tfm_ctx(tfm);
	...
	if (len < QCRYPTO_RFC3686_NONCE_LEN)
		return -EINVAL;

	len -= QCRYPTO_RFC3686_NONCE_LEN;

	memcpy(ctx->rfc3686_nonce, key + len, QCRYPTO_RFC3686_NONCE_LEN);
			...
			struct qcrypto_req_control *pqcrypto_req_control)
{
	...
	driver_name = crypto_tfm_alg_driver_name(crypto_ablkcipher_tfm(tfm));
	...
	qreq.iv = req->info;
	qreq.ivsize = strcmp(driver_name, "qcrypto-rfc3686-ctr-aes") == 0 ?
		AES_BLOCK_SIZE :
...
static int _qcrypto_enc_rfc3686(struct ablkcipher_request *req)
{
	struct qcrypto_cipher_req_ctx *rctx;
	struct qcrypto_cipher_ctx *ctx = crypto_tfm_ctx(req->base.tfm);
	struct crypto_priv *cp = ctx->cp;
	struct crypto_stat *pstat;
	uint32_t count = cpu_to_be32((req->base.flags & CRYPTO_TFM_REQ_MERAKI_MODE) ? 0 : 1);
	...
	BUG_ON(crypto_tfm_alg_type(req->base.tfm) !=
				CRYPTO_ALG_TYPE_ABLKCIPHER);
#ifdef QCRYPTO_DEBUG
	dev_info(&ctx->pengine->pdev->dev, "_qcrypto_enc_rfc3686: %p\n", req);
#endif
	...
	memcpy(rctx->rfc3686_iv, ctx->rfc3686_nonce, QCRYPTO_RFC3686_NONCE_LEN);
	memcpy(rctx->rfc3686_iv + QCRYPTO_RFC3686_NONCE_LEN, req->info, QCRYPTO_RFC3686_IV_LEN);
	memcpy(rctx->rfc3686_iv + QCRYPTO_RFC3686_NONCE_LEN + QCRYPTO_RFC3686_IV_LEN,
		&count, sizeof(count));
	rctx->iv = rctx->rfc3686_iv;
...
static int _qcrypto_dec_rfc3686(struct ablkcipher_request *req)
{
	struct qcrypto_cipher_req_ctx *rctx;
	struct qcrypto_cipher_ctx *ctx = crypto_tfm_ctx(req->base.tfm);
	struct crypto_priv *cp = ctx->cp;
	...
	BUG_ON(crypto_tfm_alg_type(req->base.tfm) !=
                                        CRYPTO_ALG_TYPE_ABLKCIPHER);
#ifdef QCRYPTO_DEBUG
	dev_info(&ctx->pengine->pdev->dev, "_qcrypto_dec_rfc3686: %p\n", req);
#endif
...
static struct crypto_alg _qcrypto_ablk_cipher_algos[] = {
	...
	},
	{
		...
		.cra_driver_name = "qcrypto-rfc3686-ctr-aes",
		.cra_priority	= 300,
		.cra_flags	= CRYPTO_ALG_TYPE_ABLKCIPHER |
					CRYPTO_ALG_NEED_FALLBACK |
					CRYPTO_ALG_ASYNC,
		...
		.cra_u		= {
			.ablkcipher = {
				.ivsize		= QCRYPTO_RFC3686_IV_LEN,
				.min_keysize	= AES_MIN_KEY_SIZE,
				.max_keysize	= AES_MAX_KEY_SIZE,
				.setkey		= _qcrypto_setkey_rfc3686,
				.encrypt	= _qcrypto_enc_rfc3686,
				.decrypt	= _qcrypto_dec_rfc3686,
			},
		},
	},
	{
...
#if 0
	/* Disabled for performance reasons on insects after extensive testing.
	 * Use SW hashing instead.
	 */
