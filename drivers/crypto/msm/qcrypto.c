...
struct qcrypto_cipher_ctx {
	...
	u8 rfc3686_nonce[RFC3686_NONCE_SIZE];
...
struct qcrypto_cipher_req_ctx {
	...
	u8 rfc3686_iv[QCRYPTO_MAX_IV_LENGTH + RFC3686_NONCE_SIZE];
	unsigned int ivsize;
	int  aead;
...
#define	MSM_QCRYPTO_REQ_QUEUE_LENGTH 608
		...
		unsigned int len)
{
	...
	if (len < RFC3686_NONCE_SIZE)
	    return -EINVAL;

	len -= RFC3686_NONCE_SIZE;

	memcpy(ctx->rfc3686_nonce, key + len, RFC3686_NONCE_SIZE);
...
static int _qcrypto_enc_rfc3686(struct ablkcipher_request *req)
{
	...
	memset(rctx->rfc3686_iv, 0, sizeof(rctx->rfc3686_iv));
	memcpy(rctx->rfc3686_iv, ctx->rfc3686_nonce, RFC3686_NONCE_SIZE);
	memcpy(rctx->rfc3686_iv + RFC3686_NONCE_SIZE, req->info, 8);
...
static int  _qcrypto_probe(struct platform_device *pdev)
{
	...
	/* Disabled for performance reasons on insects. Use SW hashing instead.
