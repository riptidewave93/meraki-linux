...
static ssize_t set_freq(struct device *dev, struct device_attribute *attr,
    ...
    if (strict_strtoul(buf, 10, &freq) || freq > MAX_FREQ)
        return -EINVAL;
...
static ssize_t set_duty(struct device *dev, struct device_attribute *attr,
    ...
    if (strict_strtoul(buf, 10, &duty) || duty > MAX_DUTY)
        return -EINVAL;
...
static ssize_t set_polarity(struct device *dev, struct device_attribute *attr,
    ...
    if (strict_strtoul(buf, 2, &polarity))
        return -EINVAL;
...
static ssize_t set_gating(struct device *dev, struct device_attribute *attr,
    ...
    if (strict_strtoul(buf, 2, &gating))
        return -EINVAL;
...
static ssize_t set_open_collector(struct device *dev, struct device_attribute *attr,
    ...
    if (strict_strtoul(buf, 2, &open_collector))
        return -EINVAL;
...
static const struct attribute_group vcoreiii_pwm_fan_group = {
    .attrs = vcoreiii_pwm_fan_attrs,
};
...
static int vcoreiii_pwm_fan_probe(struct platform_device *pdev)
{
    ...
    ret = sysfs_create_group(&pdev->dev.kobj, &vcoreiii_pwm_fan_group);
    if (ret)
        return ret;
    ...
    if (IS_ERR(hwmon)) {
        ...
        sysfs_remove_group(&pdev->dev.kobj, &vcoreiii_pwm_fan_group);
...
static int __init vcoreiii_pwm_fan_init(void)
{
    platform_driver_register(&vcoreiii_pwm_fan_driver);
}

static void __exit vcoreiii_pwm_fan_exit(void)
{
    platform_driver_unregister(&vcoreiii_pwm_fan_driver);
}
...
module_init(vcoreiii_pwm_fan_init);
module_exit(vcoreiii_pwm_fan_exit);
