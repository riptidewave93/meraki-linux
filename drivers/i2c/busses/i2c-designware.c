...
static int __devinit dw_i2c_probe(struct platform_device *pdev)
{
	...
	{
		u32 param1 = dev->read(dev->base + DW_IC_COMP_PARAM_1);
	...
	dev->write(0, dev->base + DW_IC_INTR_MASK); /* disable IRQ */
...
#else /* of device support */

static int __devinit dw_i2c_of_probe(struct of_device *op,
				   const struct of_device_id *match)
{
	...
	devid = of_get_property(op->node, "bus_num", NULL);
	if (*devid == 0) {
		i2c->clk = clk_get(&op->dev, "i2c0");
		i2c->write = writel_be;
		i2c->read = readl_be;
	} else {
		i2c->clk = clk_get(&op->dev, "i2c1");
		i2c->write = writel;
		i2c->read = readl;
	}
#else
...
static int __devexit dw_i2c_of_remove(struct of_device *op)
{
	struct dw_i2c_dev *i2c = dev_get_drvdata(&op->dev);
...
#ifdef CONFIG_PM
static int dw_of_i2c_suspend(struct of_device * op, pm_message_t state)
{
        struct dw_i2c_dev *i2c = dev_get_drvdata(&op->dev);
        /* Disable the adapter */
        out_be32(i2c->base + DW_IC_ENABLE,0);
...
static int dw_of_i2c_resume(struct of_device * op)
{
        struct dw_i2c_dev *i2c = dev_get_drvdata(&op->dev);
        i2c_dw_init(i2c);
        return 0;
...
static struct of_platform_driver dw_i2c_of_driver = {
	.match_table	= dw_i2c_of_match,
...
#else	
	return of_register_platform_driver(&dw_i2c_of_driver);
#endif	
...
#else	
	of_unregister_platform_driver(&dw_i2c_of_driver);
#endif	
}
