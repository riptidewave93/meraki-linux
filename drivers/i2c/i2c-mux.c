...
/* Return the Parent's quirk structure address.
 * Helpful in determining the message size
 */
struct i2c_adapter_quirks *i2c_mux_get_quirks(struct i2c_adapter *adap)
{
	struct i2c_mux_priv *priv = adap->algo_data;
	struct i2c_adapter *parent = priv->parent;

	if (parent->algo->get_quirks)
		return parent->algo->get_quirks(parent);
	else
		return NULL;
}
						 ...
						 void *, u32))
{
	...
	priv->algo.get_quirks = i2c_mux_get_quirks;
