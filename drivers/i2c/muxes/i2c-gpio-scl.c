...
static int __devinit i2c_gpio_scl_probe(struct platform_device *pdev)
{
	...
	for (j = 0; j < pdata->num_scl; j++) {
		...
		mux->adap[j] = i2c_add_mux_adapter(adap, mux, nr, j,
...
static int __devexit i2c_gpio_scl_remove(struct platform_device *pdev)
{
	struct i2c_gpio_scl_platform_data *pdata;
	struct i2c_adapter *adap;
	struct sclmux *mux;
...
static struct platform_driver i2c_gpio_scl_driver = {
	...
	.remove = __devexit_p(i2c_gpio_scl_remove),
};
