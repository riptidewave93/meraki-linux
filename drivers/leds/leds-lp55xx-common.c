...
/* i2c transaction retries */
#define LP55XX_I2C_RETRIES      10
...
{
	int ret, i;
	for (i = 0; i < LP55XX_I2C_RETRIES; i++) {
		ret = i2c_smbus_write_byte_data(chip->cl, reg, val);
		if (ret == 0) break;
                msleep(i);
	}
	if (ret < 0)
		dev_err(&chip->cl->dev, "lp55xx_write retries exceeded\n");
	return ret;
}
...
int lp55xx_read(struct lp55xx_chip *chip, u8 reg, u8 *val)
{
	...
	if (ret < 0) {
		dev_err(&chip->cl->dev, "lp55xx_read retries exceeded\n");
		return ret;
	}
...
int lp55xx_init_device(struct lp55xx_chip *chip)
{
	...
	#ifdef CONFIG_ARCH_MERAKI_SWITCH_ARM
		/*
		 * Every time the reset is performed, the
		 * device returns I/O error for the first
		 * subsequent read. So, before performing
		 * the value check of the ENABLE register,
		 * a dummy read is performed to bring the
		 * device to a stable state. This change
		 * specific only to Wolfcastle since the
		 * lp5521 in other platforms does not seem
		 * to exhibit this behavior.
		 */
		u8 dummy_val;
		lp55xx_read(chip, cfg->enable.addr, &dummy_val);

		/*
		 * Sleep after the dummy read.
		 */
		usleep_range(10000, 20000);
	#endif
