						      ...
						      struct device_node *np)
{
	...
	pdata->pwrsave_dis = of_property_read_bool(np, "pwrsave-dis");
	pdata->cp_mode_exists = (!of_property_read_u8(np, "cp-mode", &pdata->cp_mode));
	pdata->log_pwm_dis = of_property_read_bool(np, "log-pwm-dis");
	pdata->exec_exists = (!of_property_read_u8(np, "exec", &pdata->exec));
