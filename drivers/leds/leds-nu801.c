...
static struct led_nu801_data * __init leds_nu801_create_of(struct platform_device *pdev)
{
...
static int __exit led_nu801_remove(struct platform_device *pdev)
{
	int i;
	struct led_nu801_platform_data *pdata = pdev->dev.platform_data;
	struct led_nu801_data *controllers;
...
static struct platform_driver led_nu801_driver = {
	...
	.remove		= __exit_p(led_nu801_remove),
	.driver		= {
		.name	= "leds-nu801",
		.owner	= THIS_MODULE,
