...
{
    struct ms420_psu_data* data = (struct ms420_psu_data*)power_supply_get_drvdata(psy);
...
static int ms420_psu_probe(struct i2c_client* client,
                           const struct i2c_device_id* id)
{
    ...
    data->psy_desc.name = devm_kzalloc(&client->dev, sizeof(char) * 20, GFP_KERNEL);
    if (!data->psy_desc.name)
        return -ENOMEM;

    sprintf((char*)data->psy_desc.name, "ms420-psu.%u", data->index);
    data->psy_desc.type = POWER_SUPPLY_TYPE_MAINS;
    data->psy_desc.properties = ms420_psu_props;
    data->psy_desc.num_properties = ARRAY_SIZE(ms420_psu_props);
    data->psy_desc.get_property = ms420_psu_get_property;
    data->psy_desc.external_power_changed = ms420_psu_external_power_changed;
    data->psy_desc.set_charged = ms420_psu_set_charged;
    ...
    data->psy = power_supply_register(&client->dev, &data->psy_desc, &psy_cfg);
    if (!data->psy)
        return ret;
