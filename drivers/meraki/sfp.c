...
static ssize_t sfp_show_good(struct device* dev, struct device_attribute* attr, char* buf)
{
    struct i2c_client* client = to_i2c_client(dev);
    struct sfp_dev* sfp = i2c_get_clientdata(client);

    return scnprintf(buf, PAGE_SIZE, "%u\n", (unsigned)sfp->sfp_good);
}
static DEVICE_ATTR(good, 0444, sfp_show_good, NULL);
...
static int sfp_make_sysfs_nodes(struct device *dev)
{
    ...
    ret = device_create_bin_file(dev, &eeprom_attr);
    if (ret < 0) {
        dev_err(dev, "Unable to create eeprom bin file");
        goto fail;
    }
                               ...
                               const struct i2c_device_id* id)
{
    ...
    sfp_devs[port-1].sfp_good = false;
