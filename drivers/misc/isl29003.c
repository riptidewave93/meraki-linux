...
static int isl29003_get_lux(struct i2c_client *client)
{
	int adc_value, range, bitdepth;

	adc_value = isl29003_get_adc_value(client);
	range = isl29003_get_range(client);
	bitdepth = (4 - isl29003_get_resolution(client)) * 4;
	return (adc_value * gain_range[range]) >> bitdepth;
}
...
static ssize_t isl29003_show_adc_value(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
...
static DEVICE_ATTR(adc_value, S_IRUGO, isl29003_show_adc_value, NULL);
