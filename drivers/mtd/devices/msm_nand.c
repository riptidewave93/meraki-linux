...
static const struct of_device_id msm_nand_dt_ids[] = {
	{ .compatible = "qcom,msm-nand" },
	{ /* sentinel */ }
};
#endif

static const char *mtd_part_types[] = {
	"ofpart",
	NULL
};
		...
		if (!rawerr) { /* check for corretable errors */
			for (n = start_sector; n < cwperpage; n++) {
				...
				if (ecc_errors) {
					...
					if (ecc_errors >= supported_flash.ecc_correctability)
					{
...
#if defined(CONFIG_OF)
	ppdata.of_node = of_find_matching_node(NULL, msm_nand_dt_ids);
#endif
...
no_dual_nand_ctlr_support:
	...
	err = mtd_device_parse_register(&info->mtd,
					mtd_part_types,
					&ppdata,
					plat_data ? plat_data->parts : NULL,
					plat_data ? plat_data->nr_parts : 0);
	if (err) {
