			     ...
			     u8 *oob_buf, bool last_cw, int page)
{
	...
	if (!nandc->boot_layout && nandc->max_boot_layout_page &&
		 (page < nandc->max_boot_layout_page)) {
		uncorrectable_err_cws=0;
	}
...
static int qcom_nandc_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dn, "boot-layout-max-page", &nandc->max_boot_layout_page);
	if (ret)
		nandc->max_boot_layout_page = 0;
	...
	if (nandc->create_sys_boot_layout) {
		...
		/* MERAKI: For cryptid compatibility with old linux-qca-3.4 based firmware.
		   Making sure that /sys/devices/platform/msm_nand/boot_layout is valid. */
		if (of_machine_is_compatible("qcom,ipq8064-ap160")) {
			if (sysfs_create_link(&platform_bus.kobj, &pdev->dev.kobj, "msm_nand"))
				dev_err(&pdev->dev, "msm_nand symlink creation failed!");
		}
	}
