...
/*        chip->ecc.calculate = ufc_calculate_ecc;*/
	...
	ufc->mtd.name = kasprintf(GFP_KERNEL, "%s.%s",
			dev_name(&ufc->ofdev->dev), flash_np->name);
	if (!ufc->mtd.name) {
		ret = -ENOMEM;
		goto err;
	}
	...
	ret = parse_mtd_partitions(&ufc->mtd, part_types, &ufc->parts, 0);
	if (ret < 0)
		goto err;

	if (ret == 0) {
		ret = of_mtd_parse_partitions(&ufc->ofdev->dev, flash_np,
					      &ufc->parts);
		if (ret < 0)
			goto err;
	}

	ret = mtd_device_register(&ufc->mtd, ufc->parts, ret);
...
static int __devinit ufc_probe(struct of_device *ofdev,
...
/*        struct device_node *np = ofdev->node;*/
	...
	ufc->ofdev = ofdev;
	dev_set_drvdata(&ofdev->dev, ufc);

	/* Read the reg property to get the chip select */
	reg = of_get_property(ofdev->node, "reg", &len);
	ufc->chip_select = 0;
	ufc->ufcbase = of_iomap(ofdev->node, 0);
	if (!ufc->ufcbase) {
	...
	err = ufc_chip_init(ufc, ofdev->node);
	if (err) {
		iounmap(ufc->ufcbase);
		return err;
	}
...
static int __devexit ufc_remove(struct of_device *ofdev)
{
	struct ufc_controller *ufc = dev_get_drvdata(&ofdev->dev);
...
#ifdef CONFIG_PM
static int ufc_suspend(struct of_device* dev, pm_message_t state)
{
	return 0;
}

static int ufc_resume(struct of_device* ofdev)
{
	u32 val;
	struct ufc_controller *ufc = dev_get_drvdata(&ofdev->dev);
	...
	OUTLE32((void *)(ufc->ufcbase + UFC_FLAG), UFC_FLAG_EN(nor_cs));

	return 0;
}
#else
# define ufc_suspend NULL
# define ufc_resume NULL
#endif

static struct of_platform_driver ufc_driver = {
	.driver = {
		.name	= "ufc",
	},
	.match_table = ufc_match,
	.probe = ufc_probe,
...
{
	return of_register_platform_driver(&ufc_driver);
}

static void __exit ufc_nand_exit(void)
{
	of_unregister_platform_driver(&ufc_driver);
}
