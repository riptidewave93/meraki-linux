	...
	*/
	...
	if (likely(!NV)) {
		...
		for (i = 0; i < 4 && 
			...
			skb_add_rx_frag(skb, i, tmp_page, 0, frag_size);
			ext_msg++;
		}
	}
...
static int __devinit apm_enet_of_probe(struct of_device *ofdev,
					const struct of_device_id *match)
{
	...
	priv_dev->ndev = ndev;
	priv_dev->ofdev = ofdev;
	priv_dev->node = ofdev->node;
	...
	dev_set_drvdata(&ofdev->dev, priv_dev);
	...
	apm_enet_add_sysfs(ofdev->dev.driver);
...
static int __devexit apm_enet_of_remove(struct of_device *ofdev)
{
	...
	if (!fini_done) {
		apm_enet_remove_sysfs(ofdev->dev.driver);
...
static int apm_enet_shutdown(struct of_device *ofdev)
{
	int rc = 0;
	struct apm_enet_dev_base *priv_dev;
	struct apm_data_priv *priv;
	...
	priv_dev = dev_get_drvdata(&ofdev->dev);
	priv = &priv_dev->priv;
...
static inline int apm_put_enet_rx_desc(struct apm_enet_dev_base *priv_dev)
{
	struct list_head *head = &priv_dev->head;
	struct apm_enet_rx_desc *entry;
	struct apm_qm_msg64 msg;

	priv_dev->in_poll_rx_msg_desc.msg = &msg;

	if (unlikely (apm_qm_pull_msg(&priv_dev->in_poll_rx_msg_desc) == -1)) {
		return APM_RC_ERROR;
	}
...
static inline int apm_get_enet_rx_desc(struct apm_enet_dev_base *priv_dev)
{
	struct list_head *head = &priv_dev->head;
	struct apm_enet_rx_desc *entry;
	...
	priv_dev->in_poll_rx_msg_desc.msg = &entry->msg;
	apm_enet_rx_frame(&priv_dev->in_poll_rx_msg_desc);
	list_del(&entry->node);
	kfree(entry);
...
static int apm_enet_empty_rx(struct apm_enet_dev_base *priv_dev, int budget)
{
	int cnt = 0;

	while (budget--) {
		ENET_DEBUG("Emptying RX Q before suspending\n");
		if (likely(apm_put_enet_rx_desc(priv_dev) == -1)) {
			break;
		} 
		cnt++;
	};
...
static int apm_enet_queue_rx(struct apm_enet_dev_base *priv_dev)
{
	int cnt = 0;

	while ((unlikely(apm_get_enet_rx_desc(priv_dev) != -1))) {
...
static int apm_enet_of_suspend(struct of_device* ofdev, pm_message_t state)
{
	struct apm_enet_dev_base *priv_dev;
	struct apm_data_priv *priv;
	struct net_device *ndev;
	...
	port = priv_dev->port_id;
	if (port == GIGE_PORT1) {
		/* Disable Ethernet Port1 in deepsleep/nap/sleep mode */
		apm_gmac_rx_disable(priv);
		apm_gmac_tx_disable(priv);
		return 0;
	}
#ifndef CONFIG_SMP	
	else if (!apm_enet_dev_struct.is_smp) {
		struct apm_data_priv temp_priv;
		memcpy(&temp_priv, priv, sizeof(struct apm_data_priv));
		temp_priv.mac_base_addr_v += 0x30;
		/* Disable Ethernet Port1 in deepsleep/nap/sleep mode */
		apm_gmac_rx_disable(&temp_priv);
		apm_gmac_tx_disable(&temp_priv);
	}
#endif
	ENET_DEBUG("Ethernet Suspend....\n");
	ndev = devices[port];
	...
	if (in6_dev && in6_dev->addr_list) 
		memcpy(&ipv6_addr_data[1], &(in6_dev->addr_list->addr),
		       	IPV6_ADDR_LEN);

	if (is_apm86xxx_lite())
		goto skip_apm86xxx_lite;

	priv_dev->pm_save_rx_enabled = apm_gmac_is_rx_enable(priv);
	apm_gmac_rx_disable(priv);
...
#endif
			apm_qm_pull_tx_comp_msg(priv_dev->qm_queues[core_id].comp_mb);
		}
#endif
		apm_qm_disable_mb_irq(priv_dev->in_poll_rx_msg_desc.mb_id);
#ifdef CONFIG_NAPI
		napi_disable(&priv_dev->napi);
#endif
	}

	saved_rx_msg = apm_enet_empty_rx(priv_dev, 1024);
	if (saved_rx_msg) {
		printk("ETHERNET SUSPEND: Saved %d rx frames.\n",
			saved_rx_msg);
	}
	...
	apm_gmac_change_mtu(priv, apm_enet_dev_struct.ipp_hw_mtu);
	...
	rc = ipp_send_data_msg(IPP_NETDATA_HDLR,
				ipv6_addr_data, sizeof(ipv6_addr_data));
	if (rc != APM_RC_OK) {
		printk(KERN_ERR "ETHERNET SUSPEND: "
...
static int apm_enet_of_resume(struct of_device* ofdev)
{
	int rc = 0;
	struct apm_enet_dev_base *priv_dev;
	struct apm_data_priv *priv;
	...
	port = priv_dev->port_id;
	if (port == GIGE_PORT1) {
		/* Enable Ethernet Port1 */
		apm_gmac_rx_enable(priv);
		apm_gmac_tx_enable(priv);
		return 0;
	}
	ENET_DEBUG("Ethernet Resume....\n");
	...
	if (!is_apm86xxx_lite()) {
		...
		/* Restore the Mac Frame Length  */
		apm_gmac_change_mtu(priv,
				MAX_FRAME_LEN_WR(MAX_FRAME_LEN_DEFAULT));
		...
		while (apm_qml_pull_msg(&qml_msg) != -1) {
			...
			tx_msg_desc.qid = apm_enet_dev_struct.wol_qid;
			apm_qm_push_msg(&tx_msg_desc);
		}
#endif
		restored_rx_msg = apm_enet_queue_rx(priv_dev);
		if (restored_rx_msg) {
		...
		if (netif_running(ndev)) {
			...
			napi_enable(&priv_dev->napi);
#endif
			apm_qm_enable_mb_irq(priv_dev->in_poll_rx_msg_desc.mb_id);
			netif_wake_queue(ndev);
		}
		netif_device_attach(ndev);

		/* Re-enable MAC RX if enabled before we entered deep sleep */
		if (priv_dev->pm_save_rx_enabled)
			apm_gmac_rx_enable(priv);
	}
	return rc;
}
...
static struct of_platform_driver apm_enet_of_driver = {
	.name = "apm86xxx-enet",
	.match_table = apm_enet_match,
...
{
	return of_register_platform_driver(&apm_enet_of_driver);
}

static void __exit apm_net_exit(void)
{
	of_unregister_platform_driver(&apm_enet_of_driver);
}
