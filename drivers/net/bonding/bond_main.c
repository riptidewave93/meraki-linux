...
/*
 * figure out a real device from bond device for nss flow offload use
 * bond_start_xmit() is the reference API
 */
int bond_skb_realdev_name(struct sk_buff *skb, struct net_device *dev, char *real_name)
{
	struct bonding *bond = netdev_priv(dev);
	int ret = -1;
	...
	if (bond->params.mode != BOND_MODE_XOR) {
		pr_err("%s: Error: Unsupported bonding mode %d\n",
			dev->name, mode);
		goto done;
	}
	{
		// base on bond_xmit_xor
		struct list_head *iter;
		struct slave *slave;
		int slave_id = bond_xmit_hash(bond, skb) % bond->slave_cnt;
		int i = slave_id;

		/* copy from bond_xmit_slave_id */
		/* Here we start from the slave with slave_id */
		bond_for_each_slave_rcu(bond, slave, iter) {
			if (--i < 0) {
				if (bond_slave_can_tx(slave)) {
					strlcpy(real_name, slave->dev->name, IFNAMSIZ);
					ret = 0;
					goto done;
				}
			}
