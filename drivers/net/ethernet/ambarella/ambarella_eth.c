...
#include <plat/eth.h>
#include <plat/sc_config.h>
#include <plat/gpio_map.h>
...
#define AMBETH_DMA_INTEN	(ETH_DMA_INTEN_NIE | ETH_DMA_INTEN_AIE | \
				...
				AMBETH_TXDMA_INTEN)
/*for power led use when bootup*/
static int network_led_status = 0;
/* ==========================================================================*/
struct ambeth_desc {
	u32					status;
	u32					length;
...
static LanLedOpt lan_led_opt =
	{0,	(davinci_pwmregsovly) IO_ADDRESS(DAVINCI_PWM0_BASE + 0x400)};

static int set_lan_led_status(int value)
{
	lan_led_opt.regs->cfg &= 0xFFFFFFFC;
	if (NETWORK_LED_ACTIVE_VALUE == value)	// on
	{
		lan_led_opt.regs->cfg |= 0x2;
		lan_led_opt.regs->start = 1;
	}
	else		// off
	{
		lan_led_opt.regs->cfg |= 0x1;
	}
	lan_led_opt.led_status = value ;
	
	return 0;
}
#endif

void led_timer_function(unsigned long ptr)
{
        struct net_device* netdev= (struct net_device*) ptr;
        struct ambeth_info *dev = (struct ambeth_info *)netdev_priv(netdev);
#ifdef _SC_POE_WIRED_LED_
	if(network_led_status == 0)
	{
		/*Camera still bootup*/
	...
	if (!link_status)
#else
	if (dev && dev->phydev && dev->phydev->link)
#endif
	{
#ifdef _SC_NETWORK_LED_IS_PWM_
	if(TxRx){
		if(NETWORK_LED_INACTIVE_VALUE == lan_led_opt.led_status){
			set_lan_led_status(NETWORK_LED_ACTIVE_VALUE);
		}else{
			set_lan_led_status(NETWORK_LED_INACTIVE_VALUE);
		}
		TxRx = 0;
	}else{
		if(NETWORK_LED_INACTIVE_VALUE == lan_led_opt.led_status){
...
#else
	if(TxRx){
		if(NETWORK_LED_INACTIVE_VALUE == gpio_get_value(GPIO_PIN_IDX_OUTPUT_NETWORK_GREEN_LED)){
			gpio_set_value(GPIO_PIN_IDX_OUTPUT_NETWORK_GREEN_LED, NETWORK_LED_ACTIVE_VALUE);
		}else{
			gpio_set_value(GPIO_PIN_IDX_OUTPUT_NETWORK_GREEN_LED, NETWORK_LED_INACTIVE_VALUE);
		}
		TxRx = 0;
	}else{
		if(NETWORK_LED_INACTIVE_VALUE == gpio_get_value(GPIO_PIN_IDX_OUTPUT_NETWORK_GREEN_LED)){
	...
	{
#ifdef _SC_NETWORK_LED_IS_PWM_
		if(NETWORK_LED_ACTIVE_VALUE == lan_led_opt.led_status)
		{
			set_lan_led_status(NETWORK_LED_INACTIVE_VALUE);
		}
#else
		if(NETWORK_LED_ACTIVE_VALUE == gpio_get_value(GPIO_PIN_IDX_OUTPUT_NETWORK_GREEN_LED))
		{
			gpio_set_value(GPIO_PIN_IDX_OUTPUT_NETWORK_GREEN_LED, NETWORK_LED_INACTIVE_VALUE);
		}
#endif
...
ambeth_init_phy_default:
	...
	if (ambarella_is_valid_gpio_irq(&lp->platform_info->phy_irq)) {
		...
		//AACHARYA phydev->irq_flags = lp->platform_info->phy_irq.irq_type;
	}
...
static int ambeth_open(struct net_device *ndev)
{
	...
	//gpio_direction_just_output(GPIO_PIN_IDX_OUTPUT_NETWORK_GREEN_LED);
	init_timer(&led_timer);
	led_timer.function = led_timer_function;
	led_timer.data = (unsigned long)ndev;
...
#define SET_LINK_STATUS	0x89FE
#define CHK_LINK_STATUS 0x89FD
#ifdef _SC_POE_WIRED_LED_
#define SET_LED_STATUS 0x89FC
#endif
static int ambeth_ioctl(struct net_device *ndev, struct ifreq *ifr, int ecmd)
{
	int					errorCode = 0;
	struct ambeth_info			*lp;
	int linked = 0;//1 means linked
	
#ifdef _SC_POE_WIRED_LED_
	int cur_led_status = 0; // 0 - green is power led, 1- green is network led.
	if (SET_LED_STATUS == ecmd)
	{
		if (errorCode = copy_from_user(&cur_led_status, ifr->ifr_ifru.ifru_data, sizeof(cur_led_status)))
		{
			errorCode = -EINVAL;
			goto ambeth_get_settings_exit;
		}
		network_led_status = cur_led_status;
		goto  ambeth_get_settings_exit;
	}
#endif
...
#ifdef _SC_HAS_MT7620_ONBOARD_ 
	int cur_link = 1;
	if (SET_LINK_STATUS == ecmd)
	{
		if (errorCode = copy_from_user(&cur_link, ifr->ifr_ifru.ifru_data, sizeof(cur_link)))
		{
			errorCode = -EINVAL;
			goto ambeth_get_settings_exit;
		}
		link_status = cur_link;
		goto  ambeth_get_settings_exit;
	}
#endif
	lp = (struct ambeth_info *)netdev_priv(ndev);
	if (CHK_LINK_STATUS == ecmd)
	{
		if (lp && lp->phydev && lp->phydev->link)
		{
			linked = 1;	
		}
		else
		...
		if(copy_to_user((char *)ifr->ifr_data, (char *)&linked, sizeof(linked)))
		{
			errorCode = -EINVAL;
			goto ambeth_get_settings_exit;
		}
...
extern unsigned char sc_mac_addr[6];
#ifdef _SC_HAS_MT7620_ONBOARD_ 
#ifdef _MT7620_USE_BRIDGE_
static char *boot_act = NULL;
int bootact_setup(char *str)
{
        boot_act = str;
	//printk("boot_act = %s", boot_act);
        return 1;
}

__setup("bootact=", bootact_setup);
#endif
#endif
...
#endif
	memcpy(lp->platform_info->mac_addr, sc_mac_addr, AMBETH_MAC_SIZE);
#ifdef _SC_HAS_MT7620_ONBOARD_ 
#ifdef _MT7620_USE_BRIDGE_
	if (NULL == boot_act || strcmp(boot_act, "upgrade"))
        {
		lp->platform_info->mac_addr[5]++;	
        }
#endif
#endif
