...
unsigned int adm_phyGetLinkStatus(int unit, int *link, int *fdx, ag7100_phy_speed_t *speed)
{
  *link=adm_phyIsUp(unit);
  *fdx=adm_phyIsFullDuplex(unit);
  *speed=adm_phySpeed(unit);
  return 0;
}

#ifdef AG7100_PHY_RUNTIME
struct ag7100_phy_disp_t adm_phy_disp = { adm_phySetup, adm_phyGetLinkStatus, 0 };
#endif
