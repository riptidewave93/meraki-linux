...
athrs16_phy_setup(int ethUnit)
{
    ...
    for (phyUnit=0; phyUnit < ATHR_PHY_MAX; phyUnit++) {
        ...
        //turn off auto negotiation set speed to 100mbps
        //bit 13 bit 6 are 1 - 0 w/ 6 being MSB
#if 0
        phy_reg_write(phyBase, phyAddr, 16, 0x0822);
        printk("read back %x\n ", phy_reg_read(phyBase, phyAddr, ATHR_PHY_SPEC_CONTROL));
        phy_reg_write(phyBase, phyAddr, ATHR_PHY_CONTROL, ATHR_CTRL_SOFTWARE_RESET);
        printk("read back %x\n ", phy_reg_read(phyBase, phyAddr, ATHR_PHY_SPEC_CONTROL));
        phy_reg_write(phyBase, phyAddr, ATHR_PHY_CONTROL, 0x2100);
        printk("XXXXXX  wrote phyBase %x phyAddr %x 0x2100 read back %x\n", phyBase, phyAddr, phy_reg_read(phyBase, phyAddr, 0));
#endif
        phy_reg_write(phyBase, phyAddr, ATHR_AUTONEG_ADVERT,
                ATHR16_ADVERTISE_ALL);
    ...
    for (phyUnit=0; (phyUnit < ATHR_PHY_MAX) /*&& (timeout > 0) */; phyUnit++) {
        ...
        for (;;) {
            ...
            if (timeout == 0) {
                printk("Auto negotiation timeout!\n");
                DRV_PRINT(DRV_DEBUG_PHYSETUP,
...
unsigned int
athrs16_get_link_status(int unit, int *link, int *fdx, ag7100_phy_speed_t *speed)
{
...
int
athrs16_phy_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
#ifdef FULL_FEATURE
    if (sw_ioctl(ifr->ifr_data, cmd))
        return -EOPNOTSUPP;
...
struct ag7100_phy_disp_t athrs16_phy_disp = { athrs16_phy_setup, athrs16_get_link_status, athrs16_phy_do_ioctl };
