...
unsigned int
athrs26_get_link_status(int unit, int *link, int *fdx, ag7100_phy_speed_t *speed)
{
  *link=athrs26_phy_is_up(unit);
  *fdx=athrs26_phy_is_fdx(unit);
  *speed=athrs26_phy_speed(unit);
  return 0;
}

#ifdef AG7100_PHY_RUNTIME
struct ag7100_phy_disp_t athrs26_phy_disp = { athrs26_phy_setup, athrs26_get_link_status, athrs26_phy_do_ioctl };
#endif
...
int
athrs26_phy_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
#ifdef FULL_FEATURE
