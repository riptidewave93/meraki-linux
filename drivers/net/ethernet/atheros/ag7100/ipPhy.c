...
static const int ipPhyDebug = DRV_DEBUG_PHYERROR /*| DRV_DEBUG_PHYCHANGE | DRV_DEBUG_PHYSETUP*/;
...
static BOOL ip_verifyReady(int ethUnit);
#ifdef DEBUG
...
ip_phySetup(int ethUnit)
{
    int     phyUnit;
    uint16_t  phyHwStatus;
    //uint16_t  phyReg_Spec_Ctrl_Status;//Eliot++
    uint16_t phyReg;
    uint16_t  timeout;
    int     liveLinks = 0;
    uint32_t  phyBase = 0;
    BOOL    foundPhy = FALSE;
    uint32_t  phyAddr, phyReg_16, phyReg_20;
...
#if 0
        /*Eliot Add for giga */
		phyReg_Spec_Ctrl_Status = phy_reg_read(0, 0x1, IP_SPECIFIC_CONTROL_STATUS);
		printk("phyReg_Spec_Ctrl_Status = %x\n",phyReg_Spec_Ctrl_Status);
		phyReg_Spec_Ctrl_Status = phyReg_Spec_Ctrl_Status | 0x1;//RXPHASE_SEL
		phyReg_Spec_Ctrl_Status = phyReg_Spec_Ctrl_Status | 0x2;//TXPHASE_SEL
		phy_reg_write(phyBase, phyAddr, IP_SPECIFIC_CONTROL_STATUS, phyReg_Spec_Ctrl_Status);
		printk("phyReg_Spec_Ctrl_Status = %x\n",phyReg_Spec_Ctrl_Status);
	 /*Eliot Add for giga */
#endif
	/*Eliot Add for EMI test */
                phyReg_16 = phy_reg_read(0, 0x1, 0x10);
                phyReg_16 |= (1 << 5 );
                phyReg_16 &= ~(1 << 6);
                phy_reg_write(0, 0x1, 0x10, phyReg_16);
		phyReg_20 = phy_reg_read(0, 0x1, 0x14);
		//printk("phyReg_20 = %x\n",phyReg_20);
		phyReg_20 &= ~ 0x2;//bit 1 set 0
		phyReg_20 |= 0x1;//bit 0 set 1
		phy_reg_write(phyBase, phyAddr, 0x14, phyReg_20);
		//printk("phyReg_20 = %x\n",phyReg_20);
        /*Eliot Add for EMI test */
...
ip_phyIsFullDuplex(int ethUnit)
{
    ...
    for (phyUnit=0; phyUnit < IP_PHY_MAX; phyUnit++) {
        ...
        if (ip_phyIsLinkAlive(phyUnit)) {
            ...
            phyBase = IP_PHYBASE(phyUnit);
            phyAddr = IP_PHYADDR(phyUnit);
	phyHwStatus=phy_reg_read(phyBase, phyAddr, 0x11);
	phyHwStatus = ((phyHwStatus & 0x1000) >> 12);
	//printk("ip_phyIsFullDuplex phyHwStatus= %d\n",phyHwStatus);
#if 0
...
ip_phySpeed(int ethUnit)
{
    ...
    for (phyUnit=0; phyUnit < IP_PHY_MAX; phyUnit++) {
        ...
        if (ip_phyIsLinkAlive(phyUnit)) {
            ...
            phyAddr = IP_PHYADDR(phyUnit);
#if 1
		phyHwStatus_17=phy_reg_read(phyBase, phyAddr, 0x11);
		//printk("SPEED::phyHwStatus_17= %x", phyHwStatus_17);
		    phyHwStatus_17 = ((phyHwStatus_17 & 0x6000) >> 13);
...
unsigned int
ip_phyGetLinkStatus(int unit, int *link, int *fdx, ag7100_phy_speed_t *speed)
{
  *link=ip_phyIsUp(unit);
  *fdx=ip_phyIsFullDuplex(unit);
  *speed=ip_phySpeed(unit);
  return 0;
}

#ifdef AG7100_PHY_RUNTIME
struct ag7100_phy_disp_t ip_phy_disp = { ip_phySetup, ip_phyGetLinkStatus, 0,
					 AG7100_MII_RMII
};
#endif
...
static const ipRegisterTableEntry_t ipPhyRegisterTable[] = {
...
static const int ipPhyNumRegs = sizeof(ipPhyRegisterTable) / sizeof(ipPhyRegisterTable[0]);


static const ipRegisterTableEntry_t ipPhy29GlobalRegisterTable[] = {
...
static const int ipPhy29GlobalNumRegs =
    sizeof(ipPhy29GlobalRegisterTable) / sizeof(ipPhy29GlobalRegisterTable[0]);


static const ipRegisterTableEntry_t ipPhy30GlobalRegisterTable[] = {
...
static const int ipPhy30GlobalNumRegs =
    sizeof(ipPhy30GlobalRegisterTable) / sizeof(ipPhy30GlobalRegisterTable[0]);

static const ipRegisterTableEntry_t ipPhy31GlobalRegisterTable[] = {
...
static const int ipPhy31GlobalNumRegs =
