...
unsigned int
rt_phyGetLinkStatus(int unit, int *link, int *fdx, ag7100_phy_speed_t *speed)
{
  *link=rt_phyIsUp(unit);
  *fdx=rt_phyIsFullDuplex(unit);
  *speed=rt_phySpeed(unit);
  return 0;
}

struct ag7100_phy_disp_t rt_phy_disp = { rt_phySetup, rt_phyGetLinkStatus, 0 };
