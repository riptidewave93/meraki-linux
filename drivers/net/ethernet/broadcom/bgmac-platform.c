...
static int bgmac_probe(struct platform_device *pdev)
{
	...
	regs = platform_get_resource_byname(pdev, IORESOURCE_MEM, "srab_base");
	if (regs) {
		bgmac->plat.srab_base = devm_ioremap_resource(&pdev->dev, regs);
		if (IS_ERR(bgmac->plat.srab_base))
			return PTR_ERR(bgmac->plat.srab_base);
	}
#endif
