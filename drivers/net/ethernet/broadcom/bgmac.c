...
#if !defined(CONFIG_ARCH_BCM_5301X)
...
#if defined(CONFIG_BGMAC_SRAB)

#define SIOCGETCROBORD     (SIOCDEVPRIVATE + 14)
#define SIOCSETCROBOWR     (SIOCDEVPRIVATE + 15)
#define ROBOCMD_DATASZ     16

static int bgmac_robo_ioctl(struct net_device *net_dev, struct ifreq *ifr, int cmd)
{
	u8 page, reg, size;
	u64 val;
	u32 *uvec;
	...
	if ((buf = kzalloc(ROBOCMD_DATASZ, GFP_KERNEL)) == NULL) {
		return -ENOMEM;
	}

	if (copy_from_user(buf, ifr->ifr_data, ROBOCMD_DATASZ)) {
		kfree(buf);
		return -EFAULT;
	}
	...
	reg = uvec[0] & 0xff;
	size = uvec[1];
if (size !=4) printk("\n\n%s: %s page %d reg %d size %d\n",__func__, (cmd==SIOCGETCROBORD)?"Rd":"Wr",page, reg, size);
	if (!bgmac->plat.srab_base) {
		return -EINVAL;
	}
	...
	switch (cmd) {
		case SIOCGETCROBORD:
			 val = -1;
			 b53_srab_read64(bgmac->plat.srab_base, page, reg, &val);
			 uvec[2] = val;
			 uvec[3] = val >> 32;
			 break;
		case SIOCSETCROBOWR:
			 ...
			 val <<= 32;
			 val |= uvec[2];
			 b53_srab_write64(bgmac->plat.srab_base, page, reg, val);
			 break;
		default:
			 break;
	}

	copy_to_user(ifr->ifr_data, buf, ROBOCMD_DATASZ);
	kfree(buf);
	return 0;
}
#endif
...
static int bgmac_ioctl(struct net_device *net_dev, struct ifreq *ifr, int cmd)
{
	...
	switch (cmd) {
		...
		case SIOCSETCROBOWR:
		case SIOCGETCROBORD:
			return bgmac_robo_ioctl(net_dev, ifr, cmd);
		default:
		return -EINVAL;
	}
#else
...
static int bgmac_get_speed(struct bgmac *bgmac)
{
#if defined(CONFIG_BGMAC_SRAB)
	u32 val, speed;
	b53_srab_read32(bgmac->plat.srab_base, B53_STAT_PAGE, B53_SPEED_STAT, &val);
	switch (val & 0x3) {
		case SPEED_STAT_10M:
				speed = SPEED_10;
				break;
	...
	return speed;
#else
	return bgmac->mac_speed;
#endif
}

static int bgmac_get_duplex(struct bgmac *bgmac)
{
#if defined(CONFIG_BGMAC_SRAB)
	u32 val;
	b53_srab_read32(bgmac->plat.srab_base, B53_STAT_PAGE, B53_DUPLEX_STAT_GE, &val);
	return (val & 0x1) ?  DUPLEX_FULL : DUPLEX_HALF;
#else
	return bgmac->mac_duplex;
#endif
}

static u32 bgmac_get_link(struct net_device *net_dev)
{
#if defined(CONFIG_BGMAC_SRAB)
	struct bgmac *bgmac = netdev_priv(net_dev);
	u32 val;
	b53_srab_read32(bgmac->plat.srab_base, B53_STAT_PAGE, B53_LINK_STAT, &val);
	return (val & 0x1);
#endif
	return net_dev->phydev->link;
}

static int bgmac_get_settings(struct net_device *net_dev, struct ethtool_cmd *cmd)
{
	struct bgmac *bgmac = netdev_priv(net_dev);
	u32 val=0;
	...
	ethtool_cmd_speed_set(cmd, bgmac_get_speed(bgmac));
	cmd->duplex = bgmac_get_duplex(bgmac);
	cmd->port = 0;
...
#if defined(CONFIG_BGMAC_SRAB)
		bgmac->mac_duplex = DUPLEX_FULL;
		bgmac->mac_speed  = SPEED_1000;
		phy_dev->speed = bgmac_get_speed(bgmac);
		phy_dev->duplex = bgmac_get_duplex(bgmac);
#endif
		bgmac_mac_speed(bgmac);
...
#if CONFIG_B53_ROBO
extern void bcm5301x_robo_probe(void*);
	bcm5301x_robo_probe(bgmac->plat.srab_base);
#endif
