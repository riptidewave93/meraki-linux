...
{
	struct e1000_igb_hw *hw = &adapter->hw;
	struct e1000_igb_host_mng_dhcp_cookie *mng_cookie = &hw->mng_cookie;
	...
	e1000_igb_write_vfta(hw, index, vfta);
...
static int igb_alloc_queues(struct igb_adapter *adapter)
{
	...
	for (i = 0; i < adapter->num_tx_queues; i++) {
		...
		if (adapter->hw.mac.type == e1000_igb_82575)
	...
	for (i = 0; i < adapter->num_rx_queues; i++) {
		...
		if (adapter->hw.mac.type >= e1000_igb_82576)
			set_bit(IGB_RING_FLAG_RX_SCTP_CSUM, &ring->flags);

		/* On i350, loopback VLAN packets have the tag byte-swapped. */
		if (adapter->hw.mac.type == e1000_igb_i350)
 ...
 **/
static void igb_write_ivar(struct e1000_igb_hw *hw, int msix_vector,
...
static void igb_irq_enable(struct igb_adapter *adapter)
{
	...
	if (adapter->msix_entries) {
		...
		if (adapter->vfs_allocated_count) {
			...
			if ((adapter->mdd) &&
			    (adapter->hw.mac.type == e1000_igb_i350))
				ims |= E1000_IMS_MDDET;
		}
...
static void igb_configure(struct igb_adapter *adapter)
{
	...
	e1000_igb_rx_fifo_flush_82575(&adapter->hw);
#ifdef CONFIG_NETDEVICES_MULTIQUEUE
...
{
	if (adapter->hw.phy.media_type == e1000_igb_media_type_copper)
		e1000_igb_power_up_phy(&adapter->hw);
	else
		e1000_igb_power_up_fiber_serdes_link(&adapter->hw);

	e1000_igb_phy_hw_reset(&adapter->hw);
}
...
{
	if (adapter->hw.phy.media_type == e1000_igb_media_type_copper)
		e1000_igb_power_down_phy(&adapter->hw);
	else
		e1000_igb_shutdown_fiber_serdes_link(&adapter->hw);
}
...
void igb_reset(struct igb_adapter *adapter)
{
	...
	struct e1000_igb_fc_info *fc = &hw->fc;
	...
	case e1000_igb_82580:
		...
		pba = e1000_igb_rxpbs_adjust_82580(pba);
		break;
	case e1000_igb_82576:
		pba = E1000_READ_REG(hw, E1000_RXPBS);
	...
	if ((adapter->max_frame_size > ETH_FRAME_LEN + ETH_FCS_LEN) &&
		...
		min_tx_space = (adapter->max_frame_size +
				sizeof(union e1000_igb_adv_tx_desc) -
				ETH_FCS_LEN) * 2;
	...
	e1000_igb_reset_hw(hw);
	E1000_WRITE_REG(hw, E1000_WUC, 0);

	if (e1000_igb_init_hw(hw))
		dev_err(pci_dev_to_dev(pdev), "Hardware Error\n");

	igb_init_dmac(adapter, pba);
	/* Re-initialize the thermal sensor on i350 devices. */
	if (mac->type == e1000_igb_i350 && hw->bus.func == 0) {
		/*
		 * If present, re-initialize the external thermal sensor
		 * interface.
		 */
		if (adapter->ets)
			e1000_igb_set_i2c_bb(hw);
		e1000_igb_init_thermal_sensor_thresh(hw);
	}
			       ...
			       const struct pci_device_id *ent)
{
	...
	if (hw->phy.media_type == e1000_igb_media_type_copper) {
		...
		hw->phy.ms_type = e1000_igb_ms_hw_default;
	}

	if (e1000_igb_check_reset_block(hw))
		dev_info(pci_dev_to_dev(pdev),
	...
	if (hw->mac.type >= e1000_igb_82576)
		netdev->features |= NETIF_F_SCTP_CSUM;

	adapter->en_mng_pt = e1000_igb_enable_mng_pass_thru(hw);
	...
	if (e1000_igb_validate_nvm_checksum(hw) < 0) {
	...
	e1000_igb_read_nvm(&adapter->hw, 5, 1, &adapter->fw_version);
	...
	hw->fc.requested_mode = e1000_igb_fc_default;
	hw->fc.current_mode = e1000_igb_fc_default;

	e1000_igb_validate_mdi_setting(hw);
	...
	if (hw->bus.func == 0)
		e1000_igb_read_nvm(hw, NVM_INIT_CONTROL3_PORT_A, 1, &eeprom_data);
	else if (hw->mac.type >= e1000_igb_82580)
		hw->nvm.ops.read(hw, NVM_INIT_CONTROL3_PORT_A +
		                 NVM_82580_LAN_FUNC_OFFSET(hw->bus.func), 1,
		                 &eeprom_data);
	else if (hw->bus.func == 1)
		e1000_igb_read_nvm(hw, NVM_INIT_CONTROL3_PORT_B, 1, &eeprom_data);
	...
	dev_info(pci_dev_to_dev(pdev), "%s: (PCIe:%s:%s) ",
	         netdev->name,
	         ((hw->bus.speed == e1000_igb_bus_speed_2500) ? "2.5GT/s" :
	          (hw->bus.speed == e1000_igb_bus_speed_5000) ? "5.0GT/s" :
	                                                    "unknown"),
	         ((hw->bus.width == e1000_igb_bus_width_pcie_x4) ? "Width x4\n" :
	          (hw->bus.width == e1000_igb_bus_width_pcie_x2) ? "Width x2\n" :
	          (hw->bus.width == e1000_igb_bus_width_pcie_x1) ? "Width x1\n" :
	           "unknown"));
	dev_info(pci_dev_to_dev(pdev), "%s: MAC: ", netdev->name);
	for (i = 0; i < 6; i++)
		printk("%2.2x%c", netdev->dev_addr[i], i == 5 ? '\n' : ':');

	ret_val = e1000_igb_read_pba_string(hw, pba_str, E1000_PBANUM_LENGTH);
	if (ret_val)
	...
	if (hw->mac.type == e1000_igb_i350 && hw->bus.func == 0) {
		 ...
		 */
		e1000_igb_read_nvm(hw, NVM_ETS_CFG, 1, &ets_word);
...
static int __devinit igb_sw_init(struct igb_adapter *adapter)
{
	...
	if (e1000_igb_setup_init_funcs(hw, TRUE)) {
...
int igb_setup_tx_resources(struct igb_ring *tx_ring)
{
	...
	tx_ring->size = tx_ring->count * sizeof(union e1000_igb_adv_tx_desc);
...
void igb_setup_tctl(struct igb_adapter *adapter)
{
	...
	e1000_igb_config_collision_dist(hw);
                           ...
                           struct igb_ring *ring)
{
	...
	E1000_WRITE_REG(hw, E1000_TDLEN(reg_idx),
	                ring->count * sizeof(union e1000_igb_adv_tx_desc));
...
int igb_setup_rx_resources(struct igb_ring *rx_ring)
{
	...
	desc_len = sizeof(union e1000_igb_adv_rx_desc);
...
static void igb_rlpml_set(struct igb_adapter *adapter)
{
	...
	if (adapter->vmdq_pools && hw->mac.type != e1000_igb_82575) {
		int i;
                           ...
                           struct igb_ring *ring)
{
	...
	E1000_WRITE_REG(hw, E1000_RDLEN(reg_idx),
	               ring->count * sizeof(union e1000_igb_adv_rx_desc));
	...
	if (adapter->vfs_allocated_count ||
	    (adapter->num_rx_queues > 1 &&
	     (hw->fc.requested_mode == e1000_igb_fc_none ||
	      hw->fc.requested_mode == e1000_igb_fc_rx_pause)))
...
int igb_write_mc_addr_list(struct net_device *netdev)
{
	...
	if (!count) {
		e1000_igb_update_mc_addr_list(hw, NULL, 0);
		return 0;
	}
...
#endif
	e1000_igb_update_mc_addr_list(hw, mta_list, i);
	kfree(mta_list);
...
static void igb_update_phy_info(unsigned long data)
{
	...
	e1000_igb_get_phy_info(&adapter->hw);
}
...
bool igb_has_link(struct igb_adapter *adapter)
{
	 ...
	 * false until the e1000_igb_check_for_link establishes link
	...
	case e1000_igb_media_type_internal_serdes:
		e1000_igb_check_for_link(hw);
		link_active = !hw->mac.get_link_status;
		break;
	case e1000_igb_media_type_unknown:
	default:
		break;
	}
...
static void igb_watchdog_task(struct work_struct *work)
{
	...
	if (link) {
		...
		if (!netif_carrier_ok(netdev)) {
			u32 ctrl;
			e1000_igb_get_speed_and_duplex(hw,
			                           &adapter->link_speed,
			                           &adapter->link_duplex);
...
{
	struct e1000_igb_adv_tx_context_desc *context_desc;
...
static void igb_tx_olinfo_status(struct igb_ring *tx_ring,
				 union e1000_igb_adv_tx_desc *tx_desc,
...
#ifdef CONFIG_IGB_DISABLE_PACKET_SPLIT
#ifdef IGB_PER_PKT_TIMESTAMP
	if (adapter->hw.mac.type >= e1000_igb_82580)
...
#define PHY_IDLE_ERROR_COUNT_MASK 0x00FF
	...
	if (hw->phy.media_type == e1000_igb_media_type_copper) {
		if ((adapter->link_speed == SPEED_1000) &&
		   (!e1000_igb_read_phy_reg(hw, PHY_1000T_STATUS, &phy_tmp))) {
...
static void igb_ping_all_vfs(struct igb_adapter *adapter)
{
	...
	for (i = 0 ; i < adapter->vfs_allocated_count; i++) {
		...
		e1000_igb_write_mbx(hw, &ping, 1, i);
	}
}
...
static void igb_vf_reset_msg(struct igb_adapter *adapter, u32 vf)
{
	...
	e1000_igb_write_mbx(hw, msgbuf, 3, vf);
}
...
static void igb_rcv_ack_from_vf(struct igb_adapter *adapter, u32 vf)
{
	...
	if (!(vf_data->flags & IGB_VF_FLAG_CTS) &&
		...
		e1000_igb_write_mbx(hw, &msg, 1, vf);
...
static void igb_rcv_msg_from_vf(struct igb_adapter *adapter, u32 vf)
{
	...
	retval = e1000_igb_read_mbx(hw, msgbuf, E1000_VFMAILBOX_SIZE, vf);
	...
	if (!(vf_data->flags & IGB_VF_FLAG_CTS)) {
		...
		if (time_after(jiffies, vf_data->last_nack + (2 * HZ))) {
			e1000_igb_write_mbx(hw, msgbuf, 1, vf);
			vf_data->last_nack = jiffies;
		}
		return;
	}
...
static void igb_msg_task(struct igb_adapter *adapter)
{
	...
	for (vf = 0; vf < adapter->vfs_allocated_count; vf++) {
		...
		if (!e1000_igb_check_for_rst(hw, vf))
			igb_vf_reset_event(adapter, vf);

		/* process any messages pending */
		if (!e1000_igb_check_for_msg(hw, vf))
			igb_rcv_msg_from_vf(adapter, vf);

		/* process any acks */
		if (!e1000_igb_check_for_ack(hw, vf))
...
static bool igb_clean_tx_irq(struct igb_q_vector *q_vector)
{
	...
	struct igb_tx_buffer *tx_buffer;
	union e1000_igb_adv_tx_desc *tx_desc, *eop_desc;
...
static inline void igb_rx_checksum(struct igb_ring *ring,
				   union e1000_igb_adv_rx_desc *rx_desc,
				   struct sk_buff *skb)
{
...
#ifndef CONFIG_IGB_DISABLE_PACKET_SPLIT
static inline u16 igb_get_hlen(union e1000_igb_adv_rx_desc *rx_desc)
{
...
static bool igb_clean_rx_irq(struct igb_q_vector *q_vector, int budget)
{
	...
	while (igb_test_staterr(rx_desc, E1000_RXD_STAT_DD)) {
		...
		union e1000_igb_adv_rx_desc *next_rxd;
...
static int igb_mii_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCGMIIREG:
		if (!capable(CAP_NET_ADMIN))
			return -EPERM;
		if (e1000_igb_read_phy_reg(&adapter->hw, data->reg_num & 0x1F,
				   &data->val_out))
			return -EIO;
		break;
	case SIOCSMIIREG:
			      ...
			      struct ifreq *ifr, int cmd)
{
	 ...
	 */
	if ((hw->mac.type >= e1000_igb_82580) && tsync_rx_ctl) {
...
#endif
#else /* CLICK_VLAN_FILTER */
	bool enable = false;
#endif /* CLICK_VLAN_FILTER */
...
{
	struct pci_dev *pdev = adapter->pdev;
	struct e1000_igb_mac_info *mac = &adapter->hw.mac;

	mac->autoneg = 0;

	/* Fiber NIC's only allow 1000 gbps Full duplex */
	if ((adapter->hw.phy.media_type == e1000_igb_media_type_internal_serdes ) &&
			  ...
			  bool runtime)
{
	...
	if (wufc) {
		...
		/* Allow time for pending master requests to run */
		e1000_igb_disable_pcie_master(hw);
...
static void igb_set_vf_rate_limit(struct e1000_igb_hw *hw, int vf, int tx_rate,
			int link_speed)
{
	int rf_dec, rf_int;
	u32 bcnrc_val;
...
static void igb_check_vf_rate_limit(struct igb_adapter *adapter)
{
	...
	if ((adapter->vf_rate_link_speed == 0) || 
		(adapter->hw.mac.type != e1000_igb_82576))
		return;
...
static void igb_vmm_control(struct igb_adapter *adapter)
{
	...
	if ((hw->mac.type == e1000_igb_i350) && (adapter->vfs_allocated_count) &&
	    (adapter->mdd))
		igb_enable_mdd(adapter);

	/* enable replication and loopback support */
	e1000_igb_vmdq_set_loopback_pf(hw, adapter->vfs_allocated_count ||
				   adapter->vmdq_pools);

	e1000_igb_vmdq_set_anti_spoofing_pf(hw, adapter->vfs_allocated_count ||
					adapter->vmdq_pools,
					adapter->vfs_allocated_count);
	e1000_igb_vmdq_set_replication_pf(hw, adapter->vfs_allocated_count ||
				      adapter->vmdq_pools);
}

static void igb_init_fw(struct igb_adapter *adapter) 
{
	struct e1000_igb_fw_drv_info fw_cmd;
	struct e1000_igb_hw *hw = &adapter->hw;
	int i;
	u16 mask;
	...
	if (!hw->mac.ops.acquire_swfw_sync(hw, mask)) {
		for (i = 0; i <= FW_MAX_RETRIES; i++) {
			...
			fw_cmd.hdr.checksum = e1000_igb_calculate_checksum((u8 *)&fw_cmd,
			                                           (FW_HDR_LEN +
			                                            fw_cmd.hdr.buf_len));
			 e1000_igb_host_interface_command(hw, (u8*)&fw_cmd,
			                             sizeof(fw_cmd));
