...
bool igb_thermal_present(struct kobject *kobj)
{
	 ...
	 */
	if (adapter->ets) {
		status = e1000_igb_set_i2c_bb(&(adapter->hw));
		if (status != E1000_SUCCESS)
			return false;
	}

	status = e1000_igb_init_thermal_sensor_thresh(&(adapter->hw));
	if (status != E1000_SUCCESS)
		return false; 
			   ...
			   struct kobj_attribute *attr, char *buf)
{
	...
	rc = e1000_igb_read_nvm(hw, first_word, word_count, 
					   eeprom_buff);
	if (rc != E1000_SUCCESS)
		return 0;
...
s32 igb_sysfs_get_thermal_data(struct kobject *kobj, char *buf)
{
	...
	status = e1000_igb_get_thermal_sensor_data_generic(&(adapter->hw));
	if (status != E1000_SUCCESS)
