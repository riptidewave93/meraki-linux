			       ...
			       struct ethtool_pauseparam *pause)
{
	...
	if (hw->fc.current_mode == e1000_igb_fc_rx_pause)
		pause->rx_pause = 1;
	else if (hw->fc.current_mode == e1000_igb_fc_tx_pause)
		pause->tx_pause = 1;
	else if (hw->fc.current_mode == e1000_igb_fc_full) {
		pause->rx_pause = 1;
		pause->tx_pause = 1;
	}
}
			      ...
			      struct ethtool_pauseparam *pause)
{
	...
	if (adapter->fc_autoneg == AUTONEG_ENABLE) {
		...
		retval = ((hw->phy.media_type == e1000_igb_media_type_copper) ?
			  e1000_igb_force_mac_fc(hw) : hw->mac.ops.setup_link(hw));
	}
			  ...
			  struct ethtool_eeprom *eeprom, u8 *bytes)
{
	...
	if (hw->nvm.type == e1000_igb_nvm_eeprom_spi)
		ret_val = e1000_igb_read_nvm(hw, first_word,
					 last_word - first_word + 1,
					 eeprom_buff);
	else {
		for (i = 0; i < last_word - first_word + 1; i++) {
			ret_val = e1000_igb_read_nvm(hw, first_word + i, 1,
						 &eeprom_buff[i]);
			if (ret_val)
				break;
		}
			  ...
			  struct ethtool_eeprom *eeprom, u8 *bytes)
{
	...
	if (eeprom->offset & 1) {
		...
		ret_val = e1000_igb_read_nvm(hw, first_word, 1,
					    &eeprom_buff[0]);
		ptr++;
	}
	if (((eeprom->offset + eeprom->len) & 1) && (ret_val == 0)) {
		...
		ret_val = e1000_igb_read_nvm(hw, last_word, 1,
	...
	ret_val = e1000_igb_write_nvm(hw, first_word,
				  last_word - first_word + 1, eeprom_buff);

	/* Update the checksum over the first part of the EEPROM if needed
	 * and flush shadow RAM for 82573 controllers */
	if ((ret_val == 0) && ((first_word <= NVM_CHECKSUM_REG)))
		e1000_igb_update_nvm_checksum(hw);
...
static int igb_eeprom_test(struct igb_adapter *adapter, u64 *data)
{
	...
	*data = 0;
	...
	for (i = 0; i < (NVM_CHECKSUM_REG + 1); i++) {
		if ((e1000_igb_read_nvm(&adapter->hw, i, 1, &temp)) < 0) {
			*data = 1;
			break;
		}
		checksum += temp;
...
static void igb_phy_disable_receiver(struct igb_adapter *adapter)
{
	...
	e1000_igb_write_phy_reg(hw, 29, 0x001F);
	e1000_igb_write_phy_reg(hw, 30, 0x8FFC);
	e1000_igb_write_phy_reg(hw, 29, 0x001A);
	e1000_igb_write_phy_reg(hw, 30, 0x8FF0);
}
...
static int igb_integrated_phy_loopback(struct igb_adapter *adapter)
{
	...
	if (hw->phy.type == e1000_igb_phy_m88) {
			/* Auto-MDI/MDIX Off */
			e1000_igb_write_phy_reg(hw, M88E1000_PHY_SPEC_CTRL, 0x0808);
			/* reset to update Auto-MDI/MDIX */
			e1000_igb_write_phy_reg(hw, PHY_CONTROL, 0x9140);
			/* autoneg off */
			e1000_igb_write_phy_reg(hw, PHY_CONTROL, 0x8140);
	} else {
		/* enable MII loopback */
		if (hw->phy.type == e1000_igb_phy_82580) 
			e1000_igb_write_phy_reg(hw, I82577_PHY_LBK_CTRL, 0x8041);
	}

	/* force 1000, set loopback  */
	e1000_igb_write_phy_reg(hw, PHY_CONTROL, 0x4140);
...
static void igb_loopback_cleanup(struct igb_adapter *adapter)
{
	...
	hw->mac.autoneg = TRUE;
	e1000_igb_read_phy_reg(hw, PHY_CONTROL, &phy_reg);
	if (phy_reg & MII_CR_LOOPBACK) {
		phy_reg &= ~MII_CR_LOOPBACK;
		e1000_igb_write_phy_reg(hw, PHY_CONTROL, phy_reg);
		e1000_igb_phy_commit(hw);
	}
}
...
static int igb_loopback_test(struct igb_adapter *adapter, u64 *data)
{
	 ...
	 * sessions are active */
	if (e1000_igb_check_reset_block(&adapter->hw)) {
...
static int igb_link_test(struct igb_adapter *adapter, u64 *data)
{
	...
	*data = 0;
	...
	if (adapter->hw.phy.media_type == e1000_igb_media_type_internal_serdes) {
		...
		do {
			e1000_igb_check_for_link(&adapter->hw);
                           ...
                           enum ethtool_phys_id_state state)
{
		...
		e1000_igb_blink_led(hw);
                return 2;
        case ETHTOOL_ID_ON:
                e1000_igb_led_on(hw);
                break;
        case ETHTOOL_ID_OFF:
                e1000_igb_led_off(hw);
                break;
        case ETHTOOL_ID_INACTIVE:
		e1000_igb_led_off(hw);
		e1000_igb_cleanup_led(hw);
                break;
        }
