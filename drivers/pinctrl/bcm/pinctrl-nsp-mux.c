...
/* MERAKI: Fix incompatibilities from backporting this from 4.9 to 4.4 */
#if 1
#define pinctrl_utils_free_map                 pinctrl_utils_dt_free_map
#define devm_pinctrl_register(dev, desc, chip) pinctrl_register(desc, dev, chip)
#endif
...
static const struct nsp_pin_group nsp_pin_groups[] = {
	...
	NSP_PIN_GROUP(i2c, NSP_MUX_BASE0, 4, 0x03, 0x00),
	NSP_PIN_GROUP(mdio, NSP_MUX_BASE0, 6, 0x03, 0x00),
	NSP_PIN_GROUP(gpio_b_0, NSP_MUX_BASE0, 8, 0x01, 0x00),
	NSP_PIN_GROUP(pwm0, NSP_MUX_BASE1, 0, 0x01, 0x01),
	NSP_PIN_GROUP(gpio_b_1, NSP_MUX_BASE0, 9, 0x01, 0x00),
	NSP_PIN_GROUP(pwm1, NSP_MUX_BASE1, 1, 0x01, 0x01),
	NSP_PIN_GROUP(gpio_b_2, NSP_MUX_BASE0, 10, 0x01, 0x00),
	NSP_PIN_GROUP(pwm2, NSP_MUX_BASE1, 2, 0x01, 0x01),
	NSP_PIN_GROUP(gpio_b_3, NSP_MUX_BASE0, 11, 0x01, 0x00),
	NSP_PIN_GROUP(pwm3, NSP_MUX_BASE1, 3, 0x01, 0x01),
	NSP_PIN_GROUP(uart1, NSP_MUX_BASE0, 12, 0x0f, 0x00),
	NSP_PIN_GROUP(uart2, NSP_MUX_BASE0, 16, 0x03, 0x00),
	NSP_PIN_GROUP(synce, NSP_MUX_BASE0, 18, 0x01, 0x01),
	NSP_PIN_GROUP(sata0_led, NSP_MUX_BASE0, 19, 0x01, 0x01),
	NSP_PIN_GROUP(sata1_led, NSP_MUX_BASE0, 20, 0x01, 0x01),
	NSP_PIN_GROUP(xtal_out, NSP_MUX_BASE0, 21, 0x01, 0x00),
	NSP_PIN_GROUP(sdio_pwr, NSP_MUX_BASE0, 22, 0x01, 0x00),
	NSP_PIN_GROUP(sdio_1p8v, NSP_MUX_BASE0, 23, 0x01, 0x00),
#ifndef CONFIG_ARCH_BCM_5301X
...
#ifndef CONFIG_ARCH_BCM_5301X
static inline u32 pin_to_mask(u32 val, unsigned int shift)
{
	return val << shift;
}
#else
...
{
	/*
	 * on NS chips, there is no mux control for gpios 18,19,20;
	 * iow, GPIO 21 is enabled w/ bit 18 of CRU_GPIO_CONTROL0
	 */
	if (shift < 18)
		return val << shift;
	else if (shift < 21)
			  ...
			  struct nsp_mux_log *mux_log)
{
	...
	val &= ~pin_to_mask(mask, grp->mux.shift);
	val |= pin_to_mask(grp->mux.alt, grp->mux.shift);
				   ...
				   unsigned int pin)
{
	...
	if ((val & pin_to_mask(1, pin)) != pin_to_mask(*gpio_select, pin)) {
		val &= ~pin_to_mask(1, pin);
		val |= pin_to_mask(*gpio_select, pin);
