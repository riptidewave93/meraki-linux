...
void pd690xx_reset(struct pd690xx *pd)
{
	...
	gpio_set_value(pd->disable_ports, !pd->logic_low); /* Power down */
	gpio_set_value(pd->reset, pd->logic_low); /* Reset */
...
static void pd690xx_do_periodic(struct pd690xx *pd)
{
	...
	if (pd->work_state == 0) {
		...
		if (ret < 0)
			return;
		poe->power = (ret & 0xffff) * 100; /* Return power in mW */
	...
	if (pd->work_state < poe->controllers) {
		ret = pd690xx_do_periodic_controller(pd, pd->work_state, pd->work_substate);
		if (ret < 0)
			return;
		if (++pd->work_substate == CTRL_REG_CNT)
			pd->work_substate = 0;
	} else if (pd->work_state < poe->controllers + poe->ports) {
		ret = pd690xx_do_periodic_port(pd, pd->work_state - poe->controllers, pd->work_substate);
		if (ret < 0)
			return;
...
static void pd690xx_do_work(struct work_struct *work)
{
	...
	case PD690XX_STATE_RESET:
		...
		gpio_set_value(pd->disable_ports, pd->logic_low);
		gpio_set_value(pd->reset, !pd->logic_low);
...
static int __devinit pd690xx_probe(struct i2c_client *client,
...
static int __devexit pd690xx_remove(struct i2c_client *client)
{
	struct pd690xx *pd = i2c_get_clientdata(client);

	cancel_delayed_work(&pd->work); /* Ensure no work is pending */
	gpio_set_value(pd->reset, pd->logic_low);  /* Put the device in reset */
