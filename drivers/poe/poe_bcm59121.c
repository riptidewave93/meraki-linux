...
/*
 * bcm59121_reset - Reset the system
 *
 * This routine is called after an initialization error. We put the device in
 * reset, and reinitialize.
 *
 * This routine must be called from the state machine or with
...
void bcm59121_reset(struct bcm59121 *bcm)
{
    ...
    dev_warn(&client->dev, "PoE Unexpected error, resetting");
...
static void bcm59121_do_work(struct work_struct *work)
{
    ...
    switch (bcm->state) {
        case BCM59121_STATE_RESET:
            ...
            bcm->state = BCM59121_STATE_GPIO_DISABLE;
            /* Wait at least RESET_DELAY (1500 ms) for reset to take effect */
