 ...
 *
 * Copyright (c) 2011, 2013, 2015 Cisco Meraki, Inc.
...
/* Time in MS to wait between clearing RESET or System Reset Command and reset complete */
#define BCM59121_RESET_DELAY    1500  /* 1.5 second delay, 1500 ms */

#define BCM59121_POLL_INTERVAL    (250) /* Poll frequency for chip (polls one controller or */
                                        /* port per run) */

#define BCM59121_MAX_PORTS  48

#define BCM59121_FAST_STATE_DETECT_BATCH_SIZE 6
...
static int bcm59121_read_command(struct bcm59121 *bcm, u8 cmd, u8 seq, u8 *rxbuf)
{
    ...
    if (bcm->state == BCM59121_STATE_GPIO_DISABLE) {
        dev_err(&client->dev, "%s: bcm->state is GPIO_DISABLE", __func__);
...
static int
bcm59121_systemPowerManConfig(struct bcm59121 *bcm, int bufsize, u8 *recvbuf)
{
    u8 cmdbuf[RECVBUFSIZE];
    /* Note: unlike other commands, this does not have a sequence number (seq++) */
    CMDLOAD(cmdbuf, 0x07, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF);
...
bcm59121_powerSourceConfig(struct bcm59121 *bcm, unsigned int power_dw)
{
    ...
    CMDLOAD(cmdbuf, bcm->seq++, 0x7, (power_dw >> 8) & 0xFF, (power_dw & 0xFF),
                (guard_band >> 8) & 0xFF, (guard_band & 0xFF),
                0xFF, 0xFF, 0xFF, 0xFF);
    ret = bcm59121_command(bcm, 0x18, RECVBUFSIZE, cmdbuf, rbuf);
    if (ret != 0) {
        dev_err(&bcm->client->dev, "%s Power Configuration command failed %d\n", __func__, ret);
        return -1;
    }
    if (rbuf[3] != 0) {
        dev_err(&bcm->client->dev, "%s Power Configuration command NAK %u\n", __func__, rbuf[3]);
        return -1;
    }
    return 0;
}
...
bcm59121_update_port_group_status(struct bcm59121 *bcm)
{
    ...
    if (++group >= (poe->ports/PORTSPERQUERY))
        group = 0;
    for (r = 0; r < PORTSPERQUERY; r++) { /* 8 ports per request */
        ...
        if ((s & 0xF) > 0x7)
            ...
            kp->status = bcm59121_status_to_state[s & 0x7];
        kp->class = (s >> 4) & 0x7;
...
bcm59121_update_system_info(struct bcm59121 *bcm)
{
    ...
    if (ret == 0) {
        poe->power = (rbuf[2] << 8) + rbuf[3] * 100;     /* Power in mW */
    }
    return ret;
}

static int
bcm59121_portMap(struct bcm59121 *bcm)
{
    struct poe_dev *poe = bcm->poe;
    u8 rbuf[RECVBUFSIZE];
    u8 cmdbuf[CMDBUFSIZE];
    ...
    for (i = 0; i < poe->ports; i++) {
        ...
        if (p > 8) {
            p = 1;
            ret = bcm59121_command(bcm, 0x1D, RECVBUFSIZE, cmdbuf, rbuf);
            if (ret < 0)
                dev_err(&bcm->client->dev, "%s Logical map failed %d  %d\n", __func__, i-1, ret);
            if (rbuf[3] || rbuf[5] || rbuf[7] || rbuf[9]) {
                dev_err(&bcm->client->dev, "%s Logical map NAK %d\n", __func__, i-1);
                dev_err(&bcm->client->dev, "    Returns %x %x %x %x %x %x %x %x %x %x %x %x\n",
    ...
    if (p != 1) {
        ret = bcm59121_command(bcm, 0x1D, RECVBUFSIZE, cmdbuf, rbuf);
        if (ret < 0)
            dev_err(&bcm->client->dev, "%s Cannot logical map %d %d\n", __func__, i-1, ret);
    }
    /* Enable the Port Map */
    CMDLOAD(cmdbuf, bcm->seq++, 0x1, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF);
    ret = bcm59121_command(bcm, 0x02, RECVBUFSIZE, cmdbuf, rbuf);
    if (ret < 0)
        dev_err(&bcm->client->dev, "%s Cannot enable logical map %d\n", __func__, ret);
    if (rbuf[2] != 0)
        dev_err(&bcm->client->dev, "%s Enable logical port map returns NAK %d\n",
                __func__, rbuf[2]);
...
static int bcm59121_init_device(struct bcm59121 *bcm)
{
    ...
    if (ret != 0) {
        if (ret != -EREQENXIO) {
            dev_err(&client->dev, "PoE %s Cannot get BCM59121 status - failing %d\n",
                __func__, ret);
            bcm59121_reset(bcm); /* Try Disabling and Reinabling again */
        }
    ...
    if (poe) {
        ...
        if (!poe) {
            dev_err(&client->dev, "PoE %s Cannot allocate PoE device - Failing\n", __func__);
            ret = -ENOMEM;
            goto out;
        }
    ...
    /* Setup the port map */
    ret = bcm59121_portMap(bcm);
    if (ret != 0) {
        dev_err(&client->dev, "PoE %s Cannot set Port Map %d - Failing\n",
                __func__, ret);
        goto out;
    }
...
static bool bcm59121_was_reset(struct bcm59121 *bcm)
{
    int ret;
    ...
    if (ret != 0) {
        dev_err(&bcm->client->dev, "PoE %s Cannot get BCM59121 status - failing %d\n",
            __func__, ret);
    } else if (rxbuf[4] !=1) {
        dev_err(&bcm->client->dev, "PoE %s Logical Port Mapping Disabled: assume reset %d\n",
                __func__, ret);
        r = true;
    }
...
static void bcm59121_do_periodic(struct bcm59121 *bcm)
{
    ...
    if (bcm59121_was_reset(bcm)) {
        bcm59121_stats.sysResets++;
        bcm59121_init_device(bcm);
    }
...
static void bcm59121_do_work(struct work_struct *work)
{
    ...
    switch (bcm->state) {
        ...
        case BCM59121_STATE_GPIO_DISABLE:
            ...
            /*
             * At boot-up we need to wait at least 1.5 seconds between either
             *     1. De-asserting the RESET signal (done by kernel from
             *          arch/x86/platform/meraki/bigpony.c, for bigpony)
        ...
        case BCM59121_STATE_INITIALIZING:
            ...
            if (bcm59121_init_device(bcm)) {
                dev_err(&client->dev, "BCM59121 PoE Initialization Error - failing\n");
                break;
            }
            ...
            printk(KERN_INFO "BCM59121 PoE Controller Running\n");
            /* Fall Through */
...
bcm59121_set_enable2(struct poe_port_kobj *port_kobj, bool enable, bool lockit)
{
    ...
    if (!ret && enable) {
        ret = bcm59121_detectionType(bcm, port_num);
        if (ret < 0)
            dev_err(&bcm->client->dev, "%s detectionType failed: %d", __func__, ret);
    }
...
bcm59121_get_controller_stats(struct poe_controller_kobj *kobj, ssize_t size, char *buf)
{
    ...
    ret = bcm59121_systemPowerManConfig(bcm, RECVBUFSIZE, rxbuf);
    if (ret == 0) {
        i += scnprintf(&buf[i], size - i,
            "Sys Power Mgmt Conf: MPSS %x PM Mode 0x%x Power %d Guard %d Power %d Guard %d\n",
            rxbuf[1], rxbuf[2], (rxbuf[3] << 8) + rxbuf[4], (rxbuf[5] << 8) + rxbuf[6],
            (rxbuf[7] << 8) + rxbuf[8], (rxbuf[9] << 8) + rxbuf[10]);
    }
...
static int bcm59121_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* Set direction of gpio_reset and initial value (high, de-asserted, NOT in reset) */
    ret = gpio_direction_output(bcm->gpio_reset, 1);
    if (ret < 0) {
        dev_err(&client->dev,
    ...
    bcm->state = BCM59121_STATE_GPIO_DISABLE; /* We are in reset pending state */
    bcm->init_retries = BCM59121_RESET_RETRIES;
