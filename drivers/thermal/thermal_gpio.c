...
static int __devinit thermal_gpio_probe(struct platform_device *pdev)
{
...
static int __devexit thermal_gpio_remove(struct platform_device *pdev)
{
	struct thermal_gpio_state *tgpio = platform_get_drvdata(pdev);

	gpio_set_value(tgpio->gpio, tgpio->fan_active); /* Leave it running */
	gpio_free(tgpio->gpio);
	kfree(tgpio);
...
static struct platform_driver thermal_gpio_driver = {
	...
	.remove		= __devexit_p(thermal_gpio_remove),
	.driver		= {
		.name	= "thermal-gpio",
		.owner	= THIS_MODULE,
	},
