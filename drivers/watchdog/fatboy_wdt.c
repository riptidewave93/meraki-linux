...
static void sw_wd_expired(struct work_struct *work)
{
    ...
    get_avenrun(si.loads, 0, SI_LOAD_SHIFT - FSHIFT);
    si_meminfo(&si);
...
static int fatboy_wdt_probe(struct platform_device* dev)
{
    int err;
    u8 data;
...
static int fatboy_wdt_remove(struct platform_device* dev)
{
    cancel_delayed_work(&sw_wd_expired_work);
