 ...
 * qcom-wdt.clear_reason=1 to clear the boot reason
 */
static int clear_reason = 1;
module_param(clear_reason, int, 0);
...
static int qcom_wdt_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	int ret;
	long restart_reason = 0;
	long watchdog_status = 0;
	...
	if (clear_reason) {
		__raw_writel(0, RESTART_REASON_ADDR);
	}
	watchdog_status = (readl(wdt->base + WDT_STS) & QCOM_WDT_RST_STS);
	...
	printk(KERN_ERR "restart_reason wdt:%d r:%d\n", watchdog_status, restart_reason);
	if (restart_reason == REASON_PANIC) {
	       wdt->wdd.bootstatus = 1;
	} else if (watchdog_status && (restart_reason != REASON_RESTART)) {
	       wdt->wdd.bootstatus = 1;
	} else {
