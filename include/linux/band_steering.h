...
static inline int click_start_queue(uint8_t radio, uint8_t qos) {
    return click_queue_signal(radio, qos, 1);
}

static inline int click_stop_queue(uint8_t radio, uint8_t qos) {
    return click_queue_signal(radio, qos, 0);
}
