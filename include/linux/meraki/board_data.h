...
#define BD_WDOG_GPIO    0x80000000   /* watchdog gpio is valid in nvram */
    ...
    uint16_t pad;                         /* actually pad the entire structure to a even size mod 4 */

    /* version 11 */
    char     hwRev[64];
};

#define AR531X_BD_REV10_SIZE (sizeof(struct ar531x_boarddata) - 64)

#define AR531X_BD_REV3_SIZE (sizeof(struct ar531x_boarddata) - 64 - 48)
...
static inline unsigned short calc_board_data_checksum(const struct ar531x_boarddata *bd)
{
    ...
    if (bd->rev == htons(11)) {
        size = (int)sizeof(struct ar531x_boarddata);
